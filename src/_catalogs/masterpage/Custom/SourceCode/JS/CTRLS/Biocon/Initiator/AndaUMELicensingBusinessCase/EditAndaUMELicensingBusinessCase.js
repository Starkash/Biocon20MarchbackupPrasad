appOperations.controller("EditAndaUMELicensingBusinessCaseCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $routeParams, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    if (Logics.getSharedData() == undefined) {
        $location.path('/InitiatorUSANDAUMEDash');
    } else {
        // alert("jji");
    ...
    var strStrategyUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('StrategyMaster')/items?$select=*&$filter=(Title eq '"+encodeURIComponent('Anda - Inhouse US + EU + MOW')+"')&$top=5000&$orderby=ID asc";
    ...
    var strMarketUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('MarketMaster')/items?$select=*&$&$filter=Title eq 'US' or Title eq 'MOW' or Title eq 'EU'&$orderby=ID";
    ...
    // var strOLBUCaseUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('AndaUMEBusinessCase')/items?$select=*,Title,ID&$top=1&$orderby=Id desc";
    ...
    // var strOLBULaunchUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('AndaUMELicensingLaunchDetails')/items?$select=*,Title,ID&$top=1&$orderby=Id desc";
    var strRoleMasterUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('RoleMaster')/items?$select=*,UserGroup/Id,UserGroupId/EMail,Market/Id,Market/Title,SubMarket/Id,SubMarket/Title&$expand=UserGroup,Market,SubMarket&$filter=(TemplateType eq '"+encodeURIComponent('Anda - Inhouse US +EU + MOW')+"')&$top=5000&$orderby=ID asc";
    var strAndaUMEBusinessCaseUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('AndaUMEBusinessCase')/items?&$select=Id,BusinessCaseDescription,Title,Modified,LapVersion,BusinessCaseName,Innovator/Id,Innovator/Title,InitiationDate,CaseStage/Id,CaseStage/Title,Strategy/Id,Strategy/Title,SubStrategy/Id,SubStrategy/Title,ProductCategory/Id,ProductCategory/Title,ProductName/Id,ProductName/Title,DosageForm/Id,DosageForm/Title&$expand=Strategy,CaseStage,SubStrategy,ProductCategory,Innovator,ProductName,DosageForm&$filter=ID eq '" + $scope.IntiateID + "'&$top=5000&$orderby=ID asc";
    var strAndaUMELicensingLaunchDetailsUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('AndaUMELicensingLaunchDetails')/items?&$select=Id,Title,Modified,FilingType/Id,FilingType/Title,NoofGenericFilers,LOEDate,FillingDate,LaunchDate,PartnerDetails,Currency/Id,Currency/Title,CurrentStatus/Id,CurrentStatus/Title,Market/Id,Market/Title,SubMarket/Id,SubMarket/Title,Country/Id,Country/Title,Partner/Id,Partner/Title,AndaUMEBusinessCase/Id,AndaUMEBusinessCase/Title&$expand=Market,SubMarket,Currency,FilingType,CurrentStatus,Country,Partner,AndaUMEBusinessCase&$filter=AndaUMEBusinessCase/ID eq '" + $scope.IntiateID + "'&$top=5000&$orderby=ID asc";
    ...
    var urlColl = [strStrategyUrl, strSubStrategyUrl, strProductCategoryUrl, strProductMasterUrl, strMarketUrl, strCountryUrl, strDosageUrl, strSubMarketUrl, strPartnerUrl, strRoleMasterUrl, strAndaUMEBusinessCaseUrl, strAndaUMELicensingLaunchDetailsUrl, strAndaUMESKUDetailsUrl,strSkuMasterUrl,strPackingMasterUrl,strCurrentStatusMasterUrl,strCurrencyMasterUrl, strInnovatorMasterUrl,strFillingTypeMasterUrl];
    ...
    Logics.getBatchedData(_spPageContextInfo.webAbsoluteUrl, urlColl).then(function (batchedData) {
        ...
        $scope.InLicensingBusinessCaseColl = batchedData[10].d.results;
        ...
        if ($scope.InLicensingBusinessCaseColl.length > 0) {
            $scope.BUTitle=$scope.InLicensingBusinessCaseColl[0].Title;
            ...
            $scope.ddlProductCategory = $scope.InLicensingBusinessCaseColl[0].ProductCategory.Id;
            $scope.ddlProductName = $scope.InLicensingBusinessCaseColl[0].ProductName.Id;
            $scope.ddlStrategy = $scope.InLicensingBusinessCaseColl[0].Strategy.Id;
            $scope.ddlSubStrategy = $scope.InLicensingBusinessCaseColl[0].SubStrategy.Id;
            $scope.Modified = new Date($scope.InLicensingBusinessCaseColl[0].Modified);
            $scope.VersionNo = $scope.InLicensingBusinessCaseColl[0].VersionNo;
            $scope.LapVersion = $scope.InLicensingBusinessCaseColl[0].LapVersion;
            $scope.ddlInnovator = $scope.InLicensingBusinessCaseColl[0].Innovator.Id;
        ...
        $scope.InitiationDate = (new Date().getMonth() + 1).toString() + '-' + new Date().getFullYear().toString()
         $scope.selectedContTypeDes = [];
        $scope.PrevConType = [];
        for (var n = 0; n < $scope.InLicensingBusinessCaseColl[0].DosageForm.results.length; n++) {

            $scope.selectedContTypeDes.push({

                "label": $scope.InLicensingBusinessCaseColl[0].DosageForm.results[n].Title,

                "id": $scope.InLicensingBusinessCaseColl[0].DosageForm.results[n].Id
            });
                ...
                if (cretedCountryRow.length > 0) {
                    for (var j = 0; j < cretedCountryRow.length; j++) {
                        $scope.ExistCounryRowColl.push({
                            ...
                            "NoofGenericFiller": cretedCountryRow[j].NoofGenericFilers,
                            "FilingType": cretedCountryRow[j].FilingType.Id,
            ...
            for (var i = 0; i < $scope.ExistMarketRowColl.length; i++) {
                for (var j = 0; j < $scope.ExistMarketRowColl[i].CountryRow.length; j++) {
                    var filteredSku = $scope.ILSKUColl.filter(function (item) {
                        return (item.AndaUMELicensingLaunchDetail.Id == $scope.ExistMarketRowColl[i].CountryRow[j].LaunchId);
                    });
                    if (filteredSku.length > 0) {
                        ...
                        for (var k = 0; k < filteredSku.length; k++) {
                            sku1.push({
                                ...
                                "LaunchID": filteredSku[k].AndaUMELicensingLaunchDetail.Id
                            });
                        }
        ...
        // }
            ...
            else
            {
                if (Countleng >= 0 && Countleng <= 5) 
                {
                    ...
                    {
                        if ($scope.ExistMarketRowColl[marLen].CountryRow[i].Status == "" && $scope.ExistMarketRowColl[marLen].CountryRow[i].Country == 0 && $scope.ExistMarketRowColl[marLen].CountryRow[i].LOEDate == null && $scope.ExistMarketRowColl[marLen].CountryRow[i].LaunchDate == null &&   $scope.ExistMarketRowColl[marLen].CountryRow[i].Currency == ""  ) {
                            alert('First Fill All Fields In Country')
                            return false;

                        } else if ($scope.ExistMarketRowColl[marLen].CountryRow[i].Status == "" || $scope.ExistMarketRowColl[marLen].CountryRow[i].Country == 0 || $scope.ExistMarketRowColl[marLen].CountryRow[i].LOEDate == null || $scope.ExistMarketRowColl[marLen].CountryRow[i].LaunchDate == null ||   $scope.ExistMarketRowColl[marLen].CountryRow[i].Currency == "" ) {
                            alert('First Fill All Fields In Country')
                            return false;
...
//sn now validation part commnet
        ...
        for(var i=0;i< $scope.newMarketRow1Coll.length;i++)
        {
            for(var j=0;j<$scope.newMarketRow1Coll[i].CountryRow.length;j++)
            {
                if($scope.newMarketRow1Coll[i].ddlmarket==marID && $scope.newMarketRow1Coll[i].CountryRow[j].Country == countryID)
                {
                        ...
                        if ($scope.newMarketRow1Coll[i].CountryRow[j].Status == "" && $scope.newMarketRow1Coll[i].CountryRow[j].Country == 0 && $scope.newMarketRow1Coll[i].CountryRow[j].LOEDate == null && $scope.newMarketRow1Coll[i].CountryRow[j].LaunchDate == null &&   $scope.newMarketRow1Coll[i].CountryRow[j].Currency == ""  ) {
                            alert('First Fill All Fields In Country')
                            return false;
    
                        } else if ($scope.newMarketRow1Coll[i].CountryRow[j].Status == "" || $scope.newMarketRow1Coll[i].CountryRow[j].Country == 0 || $scope.newMarketRow1Coll[i].CountryRow[j].LOEDate == null || $scope.newMarketRow1Coll[i].CountryRow[j].LaunchDate == null ||   $scope.newMarketRow1Coll[i].CountryRow[j].Currency == ""  ) {
                            alert('First Fill All Fields In Country')
                            return false;
            ...
            if ($scope.newMarketRow1Coll.length > 0) {
                ...
                if (Countleng >= 0 && Countleng <= 2) {
                    for (var i = 0; i < $scope.newMarketRow1Coll[marLen].CountryRow.length; i++) {
                        if ($scope.newMarketRow1Coll[marLen].CountryRow[i].Status == "" && $scope.newMarketRow1Coll[marLen].CountryRow[i].Country == 0 && $scope.newMarketRow1Coll[marLen].CountryRow[i].LOEDate == null && $scope.newMarketRow1Coll[marLen].CountryRow[i].LaunchDate == null &&  $scope.newMarketRow1Coll[marLen].CountryRow[i].Currency == 0  ) {
                            alert('First Fill All Fields In Country')
                            return false;

                        } else if ($scope.newMarketRow1Coll[marLen].CountryRow[i].Status == "" || $scope.newMarketRow1Coll[marLen].CountryRow[i].Country == 0 || $scope.newMarketRow1Coll[marLen].CountryRow[i].LOEDate == null || $scope.newMarketRow1Coll[marLen].CountryRow[i].LaunchDate == null ||  $scope.newMarketRow1Coll[marLen].CountryRow[i].Currency == 0  ) {
                            alert('First Fill All Fields In Country')
                            return false;
    ...
    $scope.onSubmit = function () {
        ...
        if (marklenght > 0) {
            ...
            if (countrylenght > 0) {
                ...
                for (var i = 0; i < $scope.ExistMarketRowColl[marklenght].CountryRow.length; i++) {
                    if ($scope.ExistMarketRowColl[marklenght].CountryRow[i].Status == "" && $scope.ExistMarketRowColl[marklenght].CountryRow[i].Country == 0 && $scope.ExistMarketRowColl[marklenght].CountryRow[i].LOEDate == null && $scope.ExistMarketRowColl[marklenght].CountryRow[i].LaunchDate == null &&  $scope.ExistMarketRowColl[marklenght].CountryRow[i].Currency == ""  ) {
                        alert('First Fill All Fields In Country')
                        return false;

                    } else if ($scope.ExistMarketRowColl[marklenght].CountryRow[i].Status == "" || $scope.ExistMarketRowColl[marklenght].CountryRow[i].Country == 0 || $scope.ExistMarketRowColl[marklenght].CountryRow[i].LOEDate == null || $scope.ExistMarketRowColl[marklenght].CountryRow[i].LaunchDate == null ||  $scope.ExistMarketRowColl[marklenght].CountryRow[i].Currency == "" ) {
                        alert('First Fill All Fields In Country')
                        return false;
        ...
        //Update Data into Parent List (InLicensingBusinessCase)

        updatedBUBatch.push({

            reqUrl: _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('AndaUMEBusinessCase')/items(" + $scope.IntiateID + ")",
            action: "UPDATE",
            data: {
                __metadata: {
       ...
       // var deferred = $q.defer();
        ...
        for (var z = 0; z < $scope.UpdateLaunchDetails.length; z++) {
            ...
            updateCountryBatch.push({
                reqUrl: _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('AndaUMELicensingLaunchDetails')/items(" + $scope.UpdateLaunchDetails[z].LaunchId + ")",
                action: "UPDATE",
                async: false,
                data: {
                    __metadata: {
                        type: "SP.Data.AndaUMELicensingLaunchDetailsListItem"
                    },
                    ...
                    NoofGenericFilers: $scope.UpdateLaunchDetails[z].NoofGenericFiller,
                
                    FilingTypeId: $scope.UpdateLaunchDetails[z].FilingType,

                    CurrencyId: $scope.UpdateLaunchDetails[z].Currency,
                    AndaUMEBusinessCaseId: $scope.IntiateID
                }
            });
        }
        for (var z = 0; z < $scope.InsertLaunchDetails.length; z++) {
            ...
            insertCountryBatch.push({
                reqUrl: _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('AndaUMELicensingLaunchDetails')/items",
                action: "ADD",
                async: false,
                data: {
                    __metadata: {
                    ...
                    NoofGenericFilers: $scope.InsertLaunchDetails[z].NoofGenericFiller,
                
                    FilingTypeId: $scope.InsertLaunchDetails[z].FilingType,
        ...
        if ($scope.UpdateSKUDetails.length > 0) {
            for (var z = 0; z < $scope.UpdateSKUDetails.length; z++) {
                ...
                UpdateSKUDetailsBatch.push({
                    reqUrl: _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('AndaUMESKUDetails')/items(" + $scope.UpdateSKUDetails[z].SKUIntId + ")",
                    action: "UPDATE",
                    async: false,
                    data: {
                        __metadata: {
                            type: "SP.Data.AndaUMESKUDetailsListItem"
                        },
                        ...
                        AndaUMELicensingLaunchDetailId: $scope.UpdateSKUDetails[z].LaunchID,
                        AndaUMEBusinessCaseId: $scope.IntiateID
                    }
                });
            }
        ...
        if ($scope.InsertSKUDetails.length > 0) {
            for (var z = 0; z < $scope.InsertSKUDetails.length; z++) {
                ...
                insertSKUDetailsBatch.push({
                    reqUrl: _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('AndaUMESKUDetails')/items",
                    action: "ADD",
                    async: false,
                    data: {
                        __metadata: {
                        ...
                        AndaUMELicensingLaunchDetailId: lunchID,
                        AndaUMEBusinessCaseId: $scope.IntiateID
                    }
                });
            }
        ...
        for (var z = 0; z < $scope.FinalCountryLaunchColl.length; z++) {
            ...
            insertCountryBatch.push({
                ...
                data: {
                    ...
                    NoofGenericFilers: $scope.FinalCountryLaunchColl[z].NoofGenericFiller,
        ...
        for (var z = 0; z < $scope.FinalSKUColl.length; z++) {
            ...
            insertSKUDetailsBatch.push({
                ...
                data: {
                    ...
                    AndaUMELicensingLaunchDetailId: getLaunchID[0].LunchID,
                    AndaUMEBusinessCaseId: $scope.IntiateID
                }
            });
        }
        Logics.postBatchedData(_spPageContextInfo.webAbsoluteUrl, insertSKUDetailsBatch).then(function (insertedLaunchData) {
            ...
            if (index == $scope.FinalSKUColl.length) {
               // alert("Business Case Saved Successfully!!");
                ...
                // Utilities.initiateDailogBox($scope, insertedLaunchData, alertMessage, "/InitiatorUSANDAUMEDash");
