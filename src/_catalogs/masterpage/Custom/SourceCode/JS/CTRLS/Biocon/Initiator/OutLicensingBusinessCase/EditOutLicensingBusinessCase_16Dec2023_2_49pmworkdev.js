appOperations.controller("EditOutLicensingBusinessCaseCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $routeParams, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    ...
    Logics.getBatchedData(_spPageContextInfo.webAbsoluteUrl, urlColl).then(function (batchedData) {
        ...
        $scope.deleteCountryskuArray = [];
        $scope.skuIdtodeleteArray = [];
    ...
    $scope.bindCountryRowNew = function (marketObj, ClickValue) {
        ...
        if ($scope.newMarketRow1Coll[marLen].CountryRow.length > 0 && $scope.newMarketRow1Coll[marLen].CountryRow.length <= 1) {
            $scope.newMarketRow1Coll[marLen].CountryRow[$scope.newMarketRow1Coll[marLen].CountryRow.length].HideRemoveButtonCountryRow = true;
        }
        //
        // if(marketObj!=null)
        // {
    ...
    $scope.removeMarketDetailsRowNew = function (marketRow1, $event) {
        ...
        if( $scope.ExistMarketRowColl.length==0 &&  $scope.newMarketRow1Coll.length==0)
        {

            var confirmDelete = window.confirm('Are you sure you want to Remove all Market/Country ?');
            ...
            alert('Please Click on Add Market Button to Add New Market ')
            return false;
        }
    ...
    $scope.FindDuplicatewithinArray = function toFindDuplicates(arry) {
        ...
        if (resultToReturn) {
            alert('Duplicate Market/Country exist . Please Remove Duplicate Market/Country respected');
    ...
    $scope.onSubmit = function () {
        ...
        var duplicateMarketExists=[];

        var duplicateCountryExists=[];


        if($scope.ExistMarketRowColl.length >0 || $scope.newMarketRow1Coll.length>0 ){


            for(var s=0;s<$scope.ExistMarketRowColl.length;s++){

                duplicateMarketExists.push($scope.ExistMarketRowColl[s].ddlmarket)

                for(var t=0;t<$scope.ExistMarketRowColl[s].CountryRow.length;t++){
                    duplicateCountryExists.push($scope.ExistMarketRowColl[s].CountryRow[t].Country);

                }
            }

            for (var a = 0; a < $scope.newMarketRow1Coll.length; a++) {

                duplicateMarketExists.push($scope.newMarketRow1Coll[a].ddlmarket)

                for (var t = 0; t < $scope.newMarketRow1Coll[a].CountryRow.length; t++) {
                    duplicateCountryExists.push($scope.newMarketRow1Coll[a].CountryRow[t].Country);

                }
            }
            if (duplicateMarketExists.length > 0) {
                $scope.FindDuplicatewithinArray(duplicateMarketExists)

                

            }
            if (duplicateCountryExists.length > 0) {

                $scope.FindDuplicatewithinArray(duplicateCountryExists);
       ...
       // return false;
        ...
        ///// 05 Dec 2023 -- perfect work on hard delete  sku  and country -- first remove parent then child --

        if ($scope.skuIdtodeleteArray.length > 0) {

            for (var sku = 0; sku < $scope.skuIdtodeleteArray.length; sku++) {
                $http({
                    method: 'POST',
                    url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('OutLicensingSKUDetails')/items(" + $scope.skuIdtodeleteArray[sku] + ")",
                    headers: {
                ...
                if ($scope.deleteCountryskuArray.length > 0) {

                    for (var Country = 0; Country < $scope.deleteCountryskuArray.length; Country++) {
                        $http({
                            method: 'POST',
                            url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('OutLicensingLaunchDetails')/items(" + $scope.deleteCountryskuArray[Country] + ")",
                            headers: {
                                "Accept": "application/json;odata=verbose",
                                "X-RequestDigest": $("#__REQUESTDIGEST").val(),
                                "IF-MATCH": "*",
        ...
        if (marklenght > 0) {
            ...
            // 02 Dec Duplicate market Reset Arvind ---


            //   // Market AND SUB MARKET DUPLICATE VALIDATION
            // var  tempArrayToCheckDuplicate=[];

            //   if($scope.ExistMarketRowColl.length>0 ){
            //   tempArrayToCheckDuplicate.push($scope.ExistMarketRowColl);
            //   }
            //   if($scope.newMarketRow1Coll.length>0 ){
            //     tempArrayToCheckDuplicate.push($scope.newMarketRow1Coll);
            //     }

            // $scope.newMarketRow1Coll2=tempArrayToCheckDuplicate;

            // for(var e=0;e<$scope.newMarketRow1Coll2.length;e++){



            //   var DupMarketArray = [];
            //   angular.forEach($scope.newMarketRow1Coll2[e], function (value, key) {
            //   var exist = false;
            ...
            //   var DupCountryArray = [];
            //   for(var i=0;i<$scope.newMarketRow1Coll2.length;i++)
            //   {
            //   angular.forEach($scope.newMarketRow1Coll2[i].CountryRow, function (value, key) {
            //   var exist = false;
            ...
            //           $scope.newMarketRow1Coll2[i].CountryRow=[]
            //           $scope.newMarketRow1Coll2=[];
    ...
    $scope.removeSKUDetailsRowExist = function (skuRow1, $event) {
        ...
        if (confirmDelete) {
            ...
            // Get the ID of the SharePoint list item to be deleted
            var itemId = skuRow1.SKUIntId; // replace 'Id' with the actual field name that holds the item ID
            if (itemId != undefined) {
            ...
            // Make a request to SharePoint REST API for hard delete
    ...
    $scope.removeMarketDetailsRow = function (marketRow1, $event) {
        // Use window.confirm to show a confirmation dialog
        var confirmDelete = window.confirm('Are you sure you want to delete Market item?');
        ...
        if (confirmDelete) {
            ...
            for (var g = 0; g < $scope.ExistMarketRowColl.length; g++) {
                ...
                for (var h = 0; h < $scope.ExistMarketRowColl[g].CountryRow.length; h++) {
                        ...
                        if($scope.ExistMarketRowColl[g].CountryRow[h].Country==marketRow1.CountryRow[0].Country){

                            t = $scope.ExistMarketRowColl.indexOf(marketRow1);


                            $scope.ExistMarketRowColl.splice(t, 1);

                        }

                        if($scope.ExistMarketRowColl[g].CountryRow[h].Country==0 || marketRow1.CountryRow[0].Country==0){
                        ...
                        var launchId = marketRow1.CountryRow[0].LaunchId;

                        if (launchId != undefined) {
                            $scope.deleteCountryskuArray.push(launchId);


                            if (marketRow1.CountryRow[0].SKURow.length > 0) {


                                for (var t = 0; t < marketRow1.CountryRow[0].SKURow.length; t++) {
    
                                    var itemId = marketRow1.CountryRow[0].SKURow[t].SKUIntId;
                     ...
                     if (marketRow1.CountryRow[0].LaunchId == $scope.ExistMarketRowColl[g].CountryRow[h].LaunchId && marketRow1.ddlmarket==$scope.ExistMarketRowColl[g].ddlmarket && marketRow1.ddlSubMarket==$scope.ExistMarketRowColl[g].ddlSubMarket) {
                        ...
                        if ($scope.ExistMarketRowColl[g].CountryRow.length > 0 && $scope.ExistMarketRowColl[g].CountryRow.length <= 1) {
                            $scope.ExistMarketRowColl[g].CountryRow[$scope.ExistMarketRowColl[g].CountryRow.length - 1].HideAddButtonCountryRow = false;
                            $scope.ExistMarketRowColl[g].CountryRow[$scope.ExistMarketRowColl[g].CountryRow.length - 1].HideRemoveButtonCountryRow = true;
                        } else {
                            $scope.ExistMarketRowColl[g].CountryRow[$scope.ExistMarketRowColl[g].CountryRow.length - 1].HideAddButtonCountryRow = false;
                            $scope.ExistMarketRowColl[g].CountryRow[$scope.ExistMarketRowColl[g].CountryRow.length - 1].HideRemoveButtonCountryRow = false;
                        }
                ...
                if($scope.ExistMarketRowColl[g].CountryRow.length==0  ){
                    ...
                    alert('Please Click on Add Market Button to Add New Market ')
                    $scope.ExistMarketRowColl=[];
                    if($scope.ExistMarketRowColl[g].CountryRow.length==0 && $scope.newMarketRow1Coll[g].CountryRow.length==0){
                        $scope.ShowMarketBtn=true;
