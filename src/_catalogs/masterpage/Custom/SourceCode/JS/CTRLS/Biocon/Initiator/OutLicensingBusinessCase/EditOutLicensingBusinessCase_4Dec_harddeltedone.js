        ...
        for(var i=0;i< $scope.newMarketRow1Coll.length;i++)
        {
            for(var j=0;j<$scope.newMarketRow1Coll[i].CountryRow.length;j++)
            {
                if($scope.newMarketRow1Coll[i].ddlmarket==marID && $scope.newMarketRow1Coll[i].CountryRow[j].Country == countryID)
                {
                    ...
                    if($scope.newMarketRow1Coll[i].CountryRow[j].SKURow.length>=1)
                    {
                        alert("Only applicable for adding first SKU row. Please click on Add one more SKU section button");
                            return false;
                    }
                    // else{
                    //     alert("Only applicable for adding first SKU row. Please click on Add one more SKU section button");
...
//     $http({
//         method: 'POST',
//         url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('OutLicensingSKUDetails')/items(" + itemId + ")",
//         headers: {
...
$scope.removeSKUDetailsRowExist = function (skuRow1, $event) {
    ...
    var confirmDelete = window.confirm('Are you sure you want to delete this SKU?');
...
function (btn) {
    ...
    $scope.removeCountryDetailsRowExist = function (countryRow1, $event) {
        ...
        if (confirmDelete) {
            $event.currentTarget.style.display = "none";
            var leng1 = $scope.ExistMarketRowColl.length - 1;
            var Countleng1 = $scope.ExistMarketRowColl[leng1].CountryRow.length;
            ...
            itemIdLaunchID = countryRow1.LaunchId; // replace 'Id' with the actual field name that holds the item ID

            // Make a request to SharePoint REST API for hard delete
            $http({
                method: 'POST',
                url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('OutLicensingLaunchDetails')/items(" + itemId + ")",
                headers: {
                    "Accept": "application/json;odata=verbose",
                    "X-RequestDigest": $("#__REQUESTDIGEST").val(),
                    "IF-MATCH": "*",
...
//FOR ADD NEW MARKET CODE FROM ADD OL JS    
    ...
    $scope.removeMarketDetailsRow = function (marketRow1, $event) {
        ...
        if (confirmDelete) {
            ...
            var t = $scope.ExistMarketRowColl.indexOf(marketRow1);

            var itemId = marketRow1.CountryRow[0].LaunchId; // replace 'Id' with the actual field name that holds the item ID
            ...
            if ($scope.ExistMarketRowColl.length > 0 && $scope.ExistMarketRowColl.length <= 1) {
                $scope.ExistMarketRowColl[$scope.ExistMarketRowColl.length - 1].HideAddButtonMarketRow = false;
                $scope.ExistMarketRowColl[$scope.ExistMarketRowColl.length - 1].HideRemoveButtonMarketRow = true;
                $scope.showSubmit = true;
            } else {
                $scope.ExistMarketRowColl[$scope.ExistMarketRowColl.length - 1].HideAddButtonMarketRow = false;
                $scope.ExistMarketRowColl[$scope.ExistMarketRowColl.length - 1].HideRemoveButtonMarketRow = false;
            }
        } else {
            // If the user clicks "Cancel" in the confirmation dialog
            $event.currentTarget.style.display = "inline-block";
