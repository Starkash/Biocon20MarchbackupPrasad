appOperations.controller("AddAPICtrl", function ($scope, $http, $filter, $q, $timeout, $route, $routeParams, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    ...
    var strSubStrategyUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('SubStrategyMaster')/items?$select=*,Strategy/Title,Strategy/Id&$expand=Strategy&$filter=Strategy/Title eq 'API'&$orderby=ID";
   ...
   // change for  Email//var strOLBUCaseUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('APIBusinessCase')/items?$select=*,Title,ID&$top=1&$orderby=Id desc";
    var strOLBUCaseUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('APIBusinessCase')/items?&$select=Id,Title,BusinessCaseDescription,Modified,LapVersion,Counter,Editor/Id,Editor/Title,BusinessCaseName,InitiationDate,CaseStatus,CaseStage/Id,CaseStage/Title,Strategy/Id,Strategy/Title,SubStrategy/Id,SubStrategy/Title,ProductCategory/Id,ProductCategory/Title,ProductName/Id,ProductName/Title,DosageForm/Id,DosageForm/Title,Initiators/Id,Initiators/Title,Reviewers/Id,Reviewers/Title,Validators/Id,Validators/Title&$expand=Strategy,Editor,CaseStage,SubStrategy,ProductCategory,ProductName,DosageForm,Initiators,Reviewers,Validators&$top=5000&$orderby=Id desc";
    ...
    var strRoleMasterUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('RoleMaster')/items?$select=*,UserGroup/Id,UserGroupId/EMail,Market/Id,Market/Title,SubMarket/Id,SubMarket/Title&$expand=UserGroup,Market,SubMarket&$filter=TemplateType eq 'API+'&$top=5000&$orderby=ID"; // cascading
    ...
    var strBusinessCaseOLDocumentLinkUrlWorkFlow = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('APIBusinessCaseDocuments')/items?$select=*&$top=1000&$orderby=ID"; // cascading
 
   var strStrategyUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('StrategyMaster')/items?$select=*&$filter=Title eq 'API'&$top=5000&$orderby=ID";
...
//
   ...
   $scope.DuplicateCheck= function chkDuplicates(arr,justCheck){
    ...
    $scope.GetProductcode();
        ...
        //Insert Data into Parent List (APIBusinessCase)
        var strOLBusinessCaseURL = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('APIBusinessCase')/items";
        ...
        for (var z = 0; z < $scope.FinalCountryLaunchColl.length; z++) {
            ...
            insertCountryBatch.push({
                ...
                data: {
                    ...
                    APIBusinessCaseId: $scope.OLBusinessCaseIntID
                }
            });
        }
    ...
    $scope.SkuDetails = function (AddedLaunchids) {
        ...
        Logics.postBatchedData(_spPageContextInfo.webAbsoluteUrl, insertSKUDetailsBatch).then(function (insertedLaunchData) {
            ...
            if (index == $scope.FinalSKUColl.length) {
                ...
                //     + "<p>&nbsp;</p>"
                //     + "<p>Click "
                //     + "<a href='" + _spPageContextInfo.webAbsoluteUrl + "/SitePages/BusinessCase.aspx#!/ViewAPI/" + $scope.OLBUCaseColl[0].Id + "/" + $scope.BusinessCaseOLDocumentLinkUrlWorkFlowColl[0].DocID + "' target='_New'>here</a>"
