appOperations.controller("EditSingleTenderCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $routeParams, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
...
//
    ...
    // var strOLBUCaseUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('SingleBidBusinessCase')/items?$select=*,Title,ID&$top=1&$orderby=Id desc";
    ...
    var strRoleMasterUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('RoleMaster')/items?$select=*,UserGroup/Id,UserGroupId/EMail,Market/Id,Market/Title&$expand=UserGroup,Market&$filter=TemplateType eq 'Tender Bid single Scenarios'&$top=5000&$orderby=ID"; // cascading  
    var strOutLicensingBusinessCaseUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('SingleBidBusinessCase')/items?&$select=Id,Title,BusinessCaseDescription,LapVersion,Modified,BusinessCaseName,InitiationDate,CaseStage/Id,CaseStage/Title,Strategy/Id,Strategy/Title,SubStrategy/Id,SubStrategy/Title,ProductCategory/Id,ProductCategory/Title,ProductName/Id,ProductName/Title,DosageForm/Id,DosageForm/Title&$expand=Strategy,CaseStage,SubStrategy,ProductCategory,ProductName,DosageForm&$filter=ID eq '" + $scope.IntiateID + "'&$top=5000&$orderby=ID asc";
 ///   var strSingleBidLaunchDetailsUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('SingleBidLaunchDetails')/items?&$select=Id,Title,Modified,LOEDate,FillingDate,LaunchDate,PartnerDetails,Currency/Id,Currency/Title,CurrentStatus/Id,CurrentStatus/Title,Market/Id,Market/Title,SubMarket/Id,SubMarket/Title,Country/Id,Country/Title,Partner/Id,Partner/Title,SingleBidBusinessCase/Id,SingleBidBusinessCase/Title&$expand=Market,SubMarket,Currency,CurrentStatus,Country,Partner,SingleBidBusinessCase&$filter=SingleBidBusinessCase/ID eq '" + $scope.IntiateID + "'&$top=5000&$orderby=ID asc";
    ...
    var strOutLicensingSKUDetailsUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('SingleBidSKUDetails')/items?&$select=Id,Title,Modified,Quantity,Pack,SkuUnit/Id,SkuUnit/Title,PackingType/Id,PackingType/Title,SingleBidLaunchDetail/Id,SingleBidLaunchDetail/Title,SingleBidBusinessCase/Id,SingleBidBusinessCase/Title&$expand=SingleBidLaunchDetail,PackingType,SingleBidBusinessCase&$expand=SingleBidBusinessCase,SkuUnit,SingleBidLaunchDetail&$filter=SingleBidBusinessCase/ID eq '" + $scope.IntiateID + "'&$top=5000&$orderby=ID asc";
    ...
    var strBidAuthorityMasterUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('BidAuthorityMaster')/items?$select=*&$top=1000&$orderby=ID"; // cascading
    ...
    var urlColl = [strStrategyUrl, strSubStrategyUrl, strProductCategoryUrl, strProductMasterUrl, strMarketUrl, strCountryUrl, strDosageUrl, strSubMarketUrl, strPartnerUrl, strRoleMasterUrl, strOutLicensingBusinessCaseUrl, strSingleBidLaunchDetailsUrl, strOutLicensingSKUDetailsUrl,strSkuMasterUrl,strPackingMasterUrl,strCurrentStatusMasterUrl,strCurrencyMasterUrl,strBidAuthorityMasterUrl];
    Logics.getBatchedData(_spPageContextInfo.webAbsoluteUrl, urlColl).then(function (batchedData) {
        ...
        $scope.BidAuthorityColl = batchedData[17].d.results;
            ...
            for (var i = 0; i < $scope.ExistMarketRowColl.length; i++) {
                ...
                if (cretedCountryRow.length > 0) {
                    ...
                    for (var j = 0; j < cretedCountryRow.length; j++) {
                        ...
                        var loedate=null;
                        if(cretedCountryRow[j].LOEDate!=undefined && cretedCountryRow[j].LOEDate!=null && cretedCountryRow[j].LOEDate!='' )
                        {
                            loedate=new Date(cretedCountryRow[j].LOEDate)
                        }

                        var launchdate=null;
                        if(cretedCountryRow[j].LaunchDate!=undefined && cretedCountryRow[j].LaunchDate!=null && cretedCountryRow[j].LaunchDate!='' )
                        {
                        ...
                        $scope.ExistCounryRowColl.push({
                            ...
                            "BidDueDate": new Date(cretedCountryRow[j].BidDueDate),
                            "SupplyStartDate": new Date(cretedCountryRow[j].SupplyStartDate),
                            "SupplyEndDate": new Date(cretedCountryRow[j].SupplyEndDate),
                            "BidAuthority": cretedCountryRow[j].BidAuthority.Id,
            ...
            for (var i = 0; i < $scope.ExistMarketRowColl.length; i++) {
                for (var j = 0; j < $scope.ExistMarketRowColl[i].CountryRow.length; j++) {
                    var filteredSku = $scope.ILSKUColl.filter(function (item) {
                        return (item.SingleBidLaunchDetail.Id == $scope.ExistMarketRowColl[i].CountryRow[j].LaunchId);
                    });
                    if (filteredSku.length > 0) {
                        ...
                        for (var k = 0; k < filteredSku.length; k++) {
                            sku1.push({
                                ...
                                "LaunchID": filteredSku[k].SingleBidLaunchDetail.Id
                            });
                        }
        ...
        // }
            ...
            else
            {
                if (Countleng >= 0 && Countleng <= 5) 
                {
                    ...
                    {
                        if($scope.ExistMarketRowColl[marLen].CountryRow[i].Status!=3){
    ...
    $scope.addSKUNew = function (marID,countryID, index, exist) {
        ...
        for(var i=0;i< $scope.newMarketRow1Coll.length;i++)
        {
            for(var j=0;j<$scope.newMarketRow1Coll[i].CountryRow.length;j++)
            {
                if($scope.newMarketRow1Coll[i].ddlmarket==marID && $scope.newMarketRow1Coll[i].CountryRow[j].Country == countryID)
                {
                    if ($scope.newMarketRow1Coll[i].CountryRow[j].Status !=3){
        ...
        if (marklenght > 0) {
            ...
            if (countrylenght > 0) {
                ...
                for (var i = 0; i < $scope.ExistMarketRowColl[marklenght].CountryRow.length; i++) {
                    ...
                    if ($scope.ExistMarketRowColl[marklenght].CountryRow[i].Status!=3){
       ...
       // var deferred = $q.defer();
        ...
        for (var z = 0; z < $scope.UpdateLaunchDetails.length; z++) {
            ...
            updateCountryBatch.push({
                reqUrl: _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('SingleBidLaunchDetails')/items(" + $scope.UpdateLaunchDetails[z].LaunchId + ")",
                action: "UPDATE",
                async: false,
                data: {
                    __metadata: {
                    ...
                    SupplyEndDate: $scope.UpdateLaunchDetails[z].SupplyEndDate,
                    SupplyStartDate: $scope.UpdateLaunchDetails[z].SupplyStartDate,
                    BidDueDate: $scope.UpdateLaunchDetails[z].BidDueDate,
                    BidAuthorityId: $scope.UpdateLaunchDetails[z].BidAuthority,
        ...
        for (var z = 0; z < $scope.InsertLaunchDetails.length; z++) {
            ...
            insertCountryBatch.push({
                ...
                data: {
                    ...
                    SupplyEndDate: $scope.InsertLaunchDetails[z].SupplyEndDate,
                    SupplyStartDate: $scope.InsertLaunchDetails[z].SupplyStartDate,
                    BidDueDate: $scope.InsertLaunchDetails[z].BidDueDate,
                    BidAuthorityId: $scope.InsertLaunchDetails[z].BidAuthority,
        ...
        if ($scope.UpdateSKUDetails.length > 0) {
            for (var z = 0; z < $scope.UpdateSKUDetails.length; z++) {
                ...
                UpdateSKUDetailsBatch.push({
                    reqUrl: _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('SingleBidSKUDetails')/items(" + $scope.UpdateSKUDetails[z].SKUIntId + ")",
                    action: "UPDATE",
                    async: false,
                    data: {
                        __metadata: {
                        ...
                        SingleBidLaunchDetailId: $scope.UpdateSKUDetails[z].LaunchID,
            ...
            for (var z = 0; z < $scope.InsertSKUDetails.length; z++) {
                ...
                insertSKUDetailsBatch.push({
                    ...
                    data: {
                        ...
                        SingleBidLaunchDetailId: lunchID,
                        SingleBidBusinessCaseId: $scope.IntiateID
                    }
                });
            }
        ...
        Logics.postBatchedData(_spPageContextInfo.webAbsoluteUrl, insertSKUDetailsBatch).then(function (insertedLaunchData) {
            ...
            if (index == $scope.FinalSKUColl.length) {
               // alert("Business Case Saved Successfully!!");
                ...
                // Utilities.initiateDailogBox($scope, insertedLaunchData, alertMessage, "/InitiatorSingDash");
