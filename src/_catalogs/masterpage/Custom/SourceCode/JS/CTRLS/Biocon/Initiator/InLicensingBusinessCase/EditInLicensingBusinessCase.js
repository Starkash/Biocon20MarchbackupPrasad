appOperations.controller("EditInLicensingBusinessCaseCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $routeParams, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    ...
    var urlColl = [strStrategyUrl, strSubStrategyUrl, strProductCategoryUrl, strProductMasterUrl, strMarketUrl, strCountryUrl, strDosageUrl, strSubMarketUrl, strPartnerUrl, strRoleMasterUrl, strInLicensingBusinessCaseUrl, strInLicensingLaunchDetailsUrl, strInLicensingSKUDetailsUrl, strSkuMasterUrl, strPackingMasterUrl, strCurrentStatusMasterUrl, strCurrencyMasterUrl];
            ...
            for (var i = 0; i < $scope.ExistMarketRowColl.length; i++) {
                for (var j = 0; j < $scope.ExistMarketRowColl[i].CountryRow.length; j++) {
                    var filteredSku = $scope.ILSKUColl.filter(function (item) {
                        return (item.InLicensingLaunchDetail.Id == $scope.ExistMarketRowColl[i].CountryRow[j].LaunchId);
                    });
                    if (filteredSku.length > 0) {
                        ...
                        for (var k = 0; k < filteredSku.length; k++) {
                            sku1.push({
                                ...
                                "LaunchID": filteredSku[k].InLicensingLaunchDetail.Id
                            });
                        }
    ...
    $scope.LaunchSkuDetails = function () {
        ...
        for (var z = 0; z < $scope.UpdateLaunchDetails.length; z++) {
            ...
            updateCountryBatch.push({
                reqUrl: _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('InLicensingLaunchDetails')/items(" + $scope.UpdateLaunchDetails[z].LaunchId + ")",
                action: "UPDATE",
                async: false,
                data: {
                    __metadata: {
        ...
        if ($scope.UpdateSKUDetails.length > 0) {
            for (var z = 0; z < $scope.UpdateSKUDetails.length; z++) {
                ...
                UpdateSKUDetailsBatch.push({
                    reqUrl: _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('InLicensingSKUDetails')/items(" + $scope.UpdateSKUDetails[z].SKUIntId + ")",
                    action: "UPDATE",
                    async: false,
                    data: {
                        __metadata: {
                        ...
                        InLicensingLaunchDetailId: $scope.UpdateSKUDetails[z].LaunchID,
                        InLicensingBusinessCaseId: $scope.IntiateID
                    }
                });
            }
            ...
            for (var z = 0; z < $scope.InsertSKUDetails.length; z++) {
                ...
                insertSKUDetailsBatch.push({
                    ...
                    data: {
                        ...
                        InLicensingLaunchDetailId: lunchID,
                        InLicensingBusinessCaseId: $scope.IntiateID
                    }
                });
            }
    ...
    $scope.SkuDetailsNew = function (AddedLaunchids) {
        ...
        Logics.postBatchedData(_spPageContextInfo.webAbsoluteUrl, insertSKUDetailsBatch).then(function (insertedLaunchData) {
            ...
            if (index == $scope.FinalSKUColl.length) {
                ...
                // Utilities.initiateDailogBox($scope, insertedLaunchData, alertMessage, "/InitiatorILDash");
