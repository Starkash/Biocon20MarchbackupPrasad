appOperations.controller("ChangeStageCapexCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
...
$scope.BusinessCaseOLDocumentLinkUrlWorkFlowColl=$scope.BusinessCaseOLDocumentLinkUrlWorkFlowColl1;
        ...
        if($scope.CapexBusinessCaseColl.length>0){
            if ($scope.CapexBusinessCaseColl[0].CaseStatus == 'Data To Be Rework' && $scope.CapexBusinessCaseColl[0].CaseStage.Title == 'Under Validation') {
                var array = [];
	...
	$scope.BusinessCaseOLDocumentLinkUrlWorkFlowColl[s].Modified= $scope.BusinessCaseOLDocumentLinkUrlWorkFlowColl[s].Modified;
        ...
        //          return responseLine.CapexPhase.Id == $scope.OutLicensingLaunchDetailsColl[i].Id;
            ...
            Logics.addData(strCopyBusinessCaseDocumentUrl, null).then(function (response) {
                ...
                Logics.getData(strBusinessCaseDocumentVersionUrl).then(function (docResponse) {
                    ...
                    if ($scope.filteredStageColl[0].Title == "Business Case Published") {
                        ...
                        var strInlicensingLaPUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('CapexBusinessCase')/items(" + $scope.IntiateID + ")";
