appOperations.controller("APIReviewerViewCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
...
if ($scope.APIDocumentLinkColl.length > 0) {
        ...
        excelWindow.close();
            ...
            if ($scope.APIDocumentLinkColl.length > 0) {
                for (var s = 0; s < $scope.APIDocumentLinkColl.length; s++) {
                  ...
                  $scope.APIDocumentLinkColl[s].ServRel = _spPageContextInfo.webAbsoluteUrl + "/_layouts/download.aspx?SourceUrl=" + $scope.APIDocumentLinkColl[s].File.ServerRelativeUrl;
   ...
   // https://biocon.sharepoint.com/sites/SPCustomApplUAT/BusinessCaseAutomation/_api/web/GetFolderByServerRelativeUrl('/sites/SPCustomApplUAT/BusinessCaseAutomation/APIBusinessCaseDocuments')/Files('3_OLCPRS_AXTNB_Aug%20-%202023-bo-V2.0.xlsx')/LockedByUser
    ...
    $scope.InitiatorDashboard = _spPageContextInfo.webAbsoluteUrl+"/SitePages/BusinessCase.aspx#!/ReviewerAPIDash";
