...
*/
    ...
    $scope.ExistCountryRow = function (marketObj) {
        ...
        //     return (item.MarketId == marketObj);
        // });
         var filtrdCountryRow=[];var   Countleng
        for (var i = 0; i < $scope.ExistMarketRowColl.length; i++) 
        {
        ...
        if(filtrdCountryRow.length>0)
        {
               Countleng= filtrdCountryRow[0].CountryRow.length
            if (Countleng == 5) 
            {
            ...
            else
            {
                if (Countleng != 5) {
                    ...
                    $scope.ExistCounryRowColl.push({
                        ...
                        "FillingDate": null,
                        "MarketId": filtrdCountryRow[0].ddlmarket,
                        "Market": 0,
                        "SubMarketId": filtrdCountryRow[0].ddlSubMarket,
                        "Market": $scope.MarketColl,
                        "SubMarket": $scope.subMarketColl,
                        "LaunchDate": null,
                        "PartnerDesc": "",
               ...
               // filtrdCountryRow[Countleng].CountryRow = $scope.ExistCounryRowColl
                var finalcounLen = $scope.ExistMarketRowColl[marLen].CountryRow.length - 1;
                 var skulLen = $scope.ExistMarketRowColl[marLen].CountryRow[finalcounLen].SKURow.length;
    ...
    $scope.onSubmit = function () {
        ...
        {
            $scope.BcInitiateDetails.CaseStatus=="Initiated"
        }
        else if($scope.BcInitiateDetails.CaseStatus=="Data Input Stage")
        {
            $scope.BcInitiateDetails.CaseStatus=="Data Input Stage"
        }
        var updatedBUBatch = [];
        ...
        updatedBUBatch.push({
            ...
            data: {
              ...
              // , CaseStageId: $scope.StageMasterColl[0].Id,
                CaseStatus: $scope.BcInitiateDetails.CaseStatus
