appOperations.controller("EditMultipleTenderCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $routeParams, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
...
//
    ...
    // var strOLBUCaseUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('MultipleBidBusinessCase')/items?$select=*,Title,ID&$top=1&$orderby=Id desc";
    ...
    var strOutLicensingBusinessCaseUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('MultipleBidBusinessCase')/items?&$select=Id,Title,BusinessCaseDescription,Modified,LapVersion,BusinessCaseName,InitiationDate,CaseStage/Id,CaseStage/Title,Strategy/Id,Strategy/Title,SubStrategy/Id,SubStrategy/Title,ProductCategory/Id,ProductCategory/Title,ProductName/Id,ProductName/Title,DosageForm/Id,DosageForm/Title&$expand=Strategy,CaseStage,SubStrategy,ProductCategory,ProductName,DosageForm&$filter=ID eq '" + $scope.IntiateID + "'&$top=5000&$orderby=ID asc";
 ///   var strMultipleBidLaunchDetailsUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('MultipleBidLaunchDetails')/items?&$select=Id,Title,Modified,LOEDate,FillingDate,LaunchDate,PartnerDetails,Currency/Id,Currency/Title,CurrentStatus/Id,CurrentStatus/Title,Market/Id,Market/Title,SubMarket/Id,SubMarket/Title,Country/Id,Country/Title,Partner/Id,Partner/Title,MultipleBidBusinessCase/Id,MultipleBidBusinessCase/Title&$expand=Market,SubMarket,Currency,CurrentStatus,Country,Partner,MultipleBidBusinessCase&$filter=MultipleBidBusinessCase/ID eq '" + $scope.IntiateID + "'&$top=5000&$orderby=ID asc";
    ...
    var strOutLicensingSKUDetailsUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('MultipleBidSKUDetails')/items?&$select=Id,Title,Modified,Quantity,Pack,SkuUnit/Id,SkuUnit/Title,PackingType/Id,PackingType/Title,MultipleBidLaunchDetail/Id,MultipleBidLaunchDetail/Title,MultipleBidBusinessCase/Id,MultipleBidBusinessCase/Title&$expand=MultipleBidLaunchDetail,PackingType,MultipleBidBusinessCase&$expand=MultipleBidBusinessCase,SkuUnit,MultipleBidLaunchDetail&$filter=MultipleBidBusinessCase/ID eq '" + $scope.IntiateID + "'&$top=5000&$orderby=ID asc";
    ...
    var urlColl = [strStrategyUrl, strSubStrategyUrl, strProductCategoryUrl, strProductMasterUrl, strMarketUrl, strCountryUrl, strDosageUrl, strSubMarketUrl, strPartnerUrl, strRoleMasterUrl, strOutLicensingBusinessCaseUrl, strMultipleBidLaunchDetailsUrl, strOutLicensingSKUDetailsUrl,strSkuMasterUrl,strPackingMasterUrl,strCurrentStatusMasterUrl,strCurrencyMasterUrl,strBidAuthorityMasterUrl];
            ...
            for (var i = 0; i < $scope.ExistMarketRowColl.length; i++) {
                for (var j = 0; j < $scope.ExistMarketRowColl[i].CountryRow.length; j++) {
                    var filteredSku = $scope.ILSKUColl.filter(function (item) {
                        return (item.MultipleBidLaunchDetail.Id == $scope.ExistMarketRowColl[i].CountryRow[j].LaunchId);
                    });
                    if (filteredSku.length > 0) {
                        ...
                        for (var k = 0; k < filteredSku.length; k++) {
                            sku1.push({
                                ...
                                "LaunchID": filteredSku[k].MultipleBidLaunchDetail.Id
                            });
                        }
       ...
       // var deferred = $q.defer();
        ...
        for (var z = 0; z < $scope.UpdateLaunchDetails.length; z++) {
            ...
            updateCountryBatch.push({
                reqUrl: _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('MultipleBidLaunchDetails')/items(" + $scope.UpdateLaunchDetails[z].LaunchId + ")",
                action: "UPDATE",
                async: false,
                data: {
                    __metadata: {
        ...
        if ($scope.UpdateSKUDetails.length > 0) {
            for (var z = 0; z < $scope.UpdateSKUDetails.length; z++) {
                ...
                UpdateSKUDetailsBatch.push({
                    reqUrl: _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('MultipleBidSKUDetails')/items(" + $scope.UpdateSKUDetails[z].SKUIntId + ")",
                    action: "UPDATE",
                    async: false,
                    data: {
                        __metadata: {
                        ...
                        MultipleBidLaunchDetailId: $scope.UpdateSKUDetails[z].LaunchID,
            ...
            for (var z = 0; z < $scope.InsertSKUDetails.length; z++) {
                ...
                insertSKUDetailsBatch.push({
                    ...
                    data: {
                        ...
                        MultipleBidLaunchDetailId: lunchID,
                        MultipleBidBusinessCaseId: $scope.IntiateID
                    }
                });
            }
        ...
        Logics.postBatchedData(_spPageContextInfo.webAbsoluteUrl, insertSKUDetailsBatch).then(function (insertedLaunchData) {
            ...
            if (index == $scope.FinalSKUColl.length) {
               // alert("Business Case Saved Successfully!!");
                ...
                // Utilities.initiateDailogBox($scope, insertedLaunchData, alertMessage, "/InitiatorMultipleDash");
