 appOperations.controller("ViewAndaUMECtrl", function ($scope, $http, $filter, $q, $timeout, $route, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs, $routeParams) {
        ...
        if ($scope.AndaUMEBusinessCaseColl.length > 0 || $scope.AndaLaunchDetailsColl.length > 0 || $scope.AndaUMESKUDetailsColl.length > 0 || $scope.BusinessCaseANDADocumentLinkColl.length >= 0 ) {
         ...
         //   $scope.InitiationDate = new Date($scope.AndaUMEBusinessCaseColl[0].InitiationDate);
           ...
           /* if($scope.AndaUMEBusinessCaseColl.length>0){
		            ...
		            for(var a=0;a<$scope.AndaUMEBusinessCaseColl.length;a++){
		            $scope.AndaUMEBusinessCaseColl[a].Created= $scope.AndaUMEBusinessCaseColl[a].Created;
		            $scope.AndaUMEBusinessCaseColl[a].Author= $scope.AndaUMEBusinessCaseColl[a].Author.Title
		            
		            }
		            else{
		           $scope.AndaUMEBusinessCaseColl="";
		           }
...
$scope.BusinessCaseANDADocumentLinkUrlWorkFlowColl[s].Modified= $scope.BusinessCaseANDADocumentLinkUrlWorkFlowColl[s].Modified;
        ...
        for (var i = 0; i < $scope.AndaLaunchDetailsColl.length; i++) {
            $scope.AndaLaunchDetailsColl[i].AndaUMESKUDetails = $filter('filter')($scope.AndaUMESKUDetailsColl, function (responseLine) {

               // AndaUMELicensingLaunchDetail//return responseLine.OutLicensingLaunchDetail.Id == $scope.AndaLaunchDetailsColl[i].Id;
               return responseLine.AndaUMELicensingLaunchDetail.Id == $scope.AndaLaunchDetailsColl[i].Id;
          
  });
     }
        console.log( $scope.AndaLaunchDetailsColl[0].AndaUMESKUDetails);
...
``
        ...
        Logics.updateData(updateBusinessCaseRequestUrl, BcChangeStage).then(function (Changeresponse1) {
         ...
         //   var strNewBusinessCaseDocumentRelativeUrl = _spPageContextInfo.siteServerRelativeUrl + "/OutLicensingBusinessCaseDocuments/" + $scope.Title + "-V" + (Math.floor($scope.BusinessCaseANDADocumentLinkColl[0].CaseVersion) + 1).toFixed(1).toString() + ".xlsx";
            ...
            Logics.addData(strCopyBusinessCaseDocumentUrl, null).then(function (response) {
                ...
                var strBusinessCaseDocumentVersionUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getbytitle('AndaUMEBusinessCaseDocuments')/items?$select=Id,Title,VersionType,CaseStage/Id,CaseStage/Title,CaseVersion,LapVersion,AndaUMEBusinessCaseId,AndaUMEBusinessCase/Title&$expand=CaseStage,AndaUMEBusinessCase&$filter=AndaUMEBusinessCaseId eq "+ $scope.IntiateID+"&$orderby=Id desc&$top=1";
    ...
    $scope.onCreateVersion = function () {
        ...
        Logics.updateData(updateBusinessCaseRequestUrl, BcChangeStage).then(function (Changeresponse1) {
            ...
            Logics.addData(strCopyBusinessCaseDocumentUrl, null).then(function (response) {
                ...
                var strBusinessCaseDocumentVersionUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getbytitle('AndaUMEBusinessCaseDocuments')/items?$select=Id,Title,VersionType,CaseVersion,CaseStage/Id,CaseStage/Title,AndaUMEBusinessCaseId,AndaUMEBusinessCase/Title&$expand=CaseStage,AndaUMEBusinessCase&$filter=AndaUMEBusinessCaseId eq " + $scope.IntiateID + "&$orderby=Id desc&$top=1";
    ...
    var documentLibrary = "AndaUMELicenseSupportingDocument";
    ...
    for (var i = 0; i < invFile; i++) {
      ...
      {
        //update meta data
     //   var strInvoiceDocUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('AndaUMELicenseSupportingDocument')/items("+docData.data.d.ListItemAllFields.ID+")";
     var strInvoiceDocUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('AndaUMELicenseSupportingDocument')/items(" + docData.data.d.ListItemAllFields.ID + ")";
...
//appOperations.controller("ViewAndaCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
...
//     var strAndaLaunchDetailsUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('AndaUMELicensingLaunchDetails')/items?&$select=Id,Title,Modified,LOEDate,FillingDate,LaunchDate,PartnerDetails,Currency/Id,Currency/Title,CurrentStatus/Id,CurrentStatus/Title,Market/Id,Market/Title,SubMarket/Id,SubMarket/Title,Country/Id,Country/Title,Partner/Id,Partner/Title,AndaUMEBusinessCase/Id,AndaUMEBusinessCase/Title&$expand=Market,SubMarket,Currency,CurrentStatus,Country,Partner,AndaUMEBusinessCase&$filter=AndaUMEBusinessCase/ID eq '" + $scope.IntiateID + "'&$top=5000&$orderby=ID asc";
//     var strAndaSKUDetailsUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('AndaSKUDetails')/items?&$select=Id,Title,Modified,Quantity,SkuUnit/Id,SkuUnit/Title,Pack,PackingType/Id,PackingType/Title,AndaLicensingLaunchDetail/Id,AndaLicensingLaunchDetail/Title,AndaUMEBusinessCase/Id,AndaUMEBusinessCase/Title&$expand=AndaLicensingLaunchDetail,SkuUnit,PackingType,AndaUMEBusinessCase&$filter=AndaUMEBusinessCase/ID eq '" + $scope.IntiateID + "'&$top=5000&$orderby=ID asc";
...
//         if ($scope.AndaUMEBusinessCaseColl.length > 0 || $scope.AndaLaunchDetailsColl.length > 0 || $scope.AndaSKUDetailsColl.length > 0 || $scope.BusinessCaseANDADocumentLinkColl.length >= 0 ) {
