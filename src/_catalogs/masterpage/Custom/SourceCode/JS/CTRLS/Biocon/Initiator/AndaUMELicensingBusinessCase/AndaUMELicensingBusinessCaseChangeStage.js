...
$scope.BusinessCaseANDADocumentLinkUrlWorkFlowColl=$scope.BusinessCaseANDADocumentLinkUrlWorkFlowColl1
        ...
        if($scope.AndaUMEBusinessCaseColl.length>0){
            if ($scope.AndaUMEBusinessCaseColl[0].CaseStatus == 'Data To Be Rework' && $scope.AndaUMEBusinessCaseColl[0].CaseStage.Title == 'Under Validation') {
                var array = [];
            ...
            Logics.addData(strCopyBusinessCaseDocumentUrl, null).then(function (response) {
                ...
                Logics.getData(strBusinessCaseDocumentVersionUrl).then(function (docResponse) {
                  ...
                  if($scope.filteredStageColl[0].Title == "Business Case Published"){
                    ...
                    var LAPVersion = {
                        __metadata: {
                            type: "SP.Data.AndaUMEBusinessCaseListItem" //AndaUMEBusinessCase
                        },
                    ...
                    var strInlicensingLaPUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('AndaUMEBusinessCase')/items(" + $scope.IntiateID + ")";
