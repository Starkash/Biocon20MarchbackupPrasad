appOperations.controller("ChangeStageOLCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    ...
    // file lock if excel file is opened anywhere---
    ...
    var strOutLicensingBusinessCaseUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('OutLicensingBusinessCase')/items?&$select=Id,BusinessCaseDescription,Title,LapVersion,Modified,Editor/Id,Editor/Title,BusinessCaseName,InitiationDate,CaseStatus,CaseStage/Id,CaseStage/Title,Strategy/Id,Strategy/Title,SubStrategy/Id,SubStrategy/Title,ProductCategory/Id,ProductCategory/Title,ProductName/Id,ProductName/Title,DosageForm/Id,DosageForm/Title,Initiators/Id,Initiators/Title,Reviewers/Id,Reviewers/Title,Validators/Id,Validators/Title&$expand=Strategy,Editor,CaseStage,SubStrategy,ProductCategory,ProductName,DosageForm,Initiators,Reviewers,Validators&$filter=ID eq '" + $scope.IntiateID + "'&$top=5000&$orderby=Modified desc";
    ...
    var strBusinessCaseOLDocumentLinkUrlWorkFlow = _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getbytitle('OutLicensingBusinessCaseDocuments')/items?$select=*,EncodedAbsUrl,Id,VersionDescription,Title,Modified,CaseStage/Id,CaseStage/Title,Editor/Id,Editor/Title,Editor/EMail,LapVersion,Created,Author/Id,Author/Title,Author/EMail,CaseVersion,VersionType,OutLicensingBusinessCaseId,OutLicensingBusinessCase/Title,File/Name,File/ServerRelativeUrl&$expand=File,Author,CaseStage,Editor,OutLicensingBusinessCase&$filter=OutLicensingBusinessCaseId eq " + $scope.IntiateID + "&$orderby=ID desc&$top=1000";
    ...
    Logics.getBatchedData(_spPageContextInfo.webAbsoluteUrl, urlColl).then(function (batchedData) {
        ...
        // AT UNDER VALIDATION STAGE AND CASE STATUS IS DATA TO BE WORK THEN SPLICE THE ELT REVIEW--ARVIND 14-08-2023
        if ($scope.OutLicensingBusinessCaseColl.length > 0) {
            if ($scope.OutLicensingBusinessCaseColl[0].CaseStatus == 'Data To Be Rework' && $scope.OutLicensingBusinessCaseColl[0].CaseStage.Title == 'Under Validation') {
                var array = [];
        ...
        if ($scope.OutLicensingBusinessCaseColl.length > 0 || $scope.OutLicensingLaunchDetailsColl.length > 0 || $scope.OutLicensingSKUDetailsColl.length > 0 || $scope.BusinessCaseOLDocumentLinkColl.length >= 0) {
            ...
            if ($scope.BusinessCaseOLDocumentLinkColl.length > 0) {
                for (var s = 0; s < $scope.BusinessCaseOLDocumentLinkColl.length; s++) {
                    ...
                    var fname = $scope.BusinessCaseOLDocumentLinkColl[s].Fname;
    ...
    $scope.CheckExcelFileOpenOrNot = function (param) {
        var fname = $scope.BusinessCaseOLDocumentLinkColl[0].File.Name;
        var serrel = $scope.BusinessCaseOLDocumentLinkColl[0].File.ServerRelativeUrl;
        ...
        https://biocon.sharepoint.com/sites/SPCustomApplDev/BusinessCaseAutomation/_api/web/GetFolderByServerRelativeUrl('/sites/SPCustomApplDev/BusinessCaseAutomation/OutLicensingBusinessCaseDocuments')/Files('5_OLCPRS_ATRVS_Aug%20-%202023-csCsd-V6.0.xlsx')/LockedByUser
        // https://biocon.sharepoint.com/sites/SPCustomApplUAT/BusinessCaseAutomation/_api/web/GetFolderByServerRelativeUrl('/sites/SPCustomApplUAT/BusinessCaseAutomation/OutLicensingBusinessCaseDocuments')/Files('3_OLCPRS_AXTNB_Aug%20-%202023-bo-V2.0.xlsx')/LockedByUser
        Logics.getBatchedData(_spPageContextInfo.webAbsoluteUrl, urlColl).then(function (batchedData) {
    ...
    $scope.ddlStageReuquired = function (a) {
        ...
        $scope.onInitiationSubmit = function () {
            // Logics.sendEmailToGroups($scope.InitiatorsGroupsName, [], "TestSubject","Testbody");
            //return false;
            ...
            Logics.updateData(updateBusinessCaseRequestUrl, BcChangeStage).then(function (Changeresponse1) {
                ...
                var strNewBusinessCaseDocumentRelativeUrl = _spPageContextInfo.webServerRelativeUrl + "/OutLicensingBusinessCaseDocuments/" + Title + "-V" + (Math.floor($scope.BusinessCaseOLDocumentLinkColl[0].CaseVersion) + 1).toFixed(1).toString() + ".xlsx";
                ...
                Logics.addData(strCopyBusinessCaseDocumentUrl, null).then(function (response) {
                    ...
                    Logics.getData(strBusinessCaseDocumentVersionUrl).then(function (docResponse) {
                        ...
                        if ($scope.filteredStageColl[0].Title == "Business Case Published") {
                            ...
                            var strOutlicensingLaPUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('OutLicensingBusinessCase')/items(" + $scope.IntiateID + ")";

                            Logics.updateData(strOutlicensingLaPUrl, LAPVersion).then(function (Vresponse) {
    ...
    $scope.onCreateVersion = function () {
        ...
        if ($scope.filteredStageColl[0].Title != "Minor Changes") {
            ...
            alert('This is only applicable to Minor Changes ')
            return false;
        ...
        Logics.updateData(updateBusinessCaseRequestUrl, BcChangeStage).then(function (Changeresponse1) {
            ...
            var strNewBusinessCaseDocumentRelativeUrl = _spPageContextInfo.webServerRelativeUrl + "/OutLicensingBusinessCaseDocuments/" + Title + "-V" + (parseFloat($scope.BusinessCaseOLDocumentLinkColl[0].CaseVersion + 0.1)).toFixed(1).toString() + ".xlsx";
