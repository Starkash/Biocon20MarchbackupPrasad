appOperations.controller("AddCapexBusinessCaseCtrl1", function ($scope, $http, $filter, $q, $timeout, $route, $routeParams, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
...
$scope.removeAuditItem1 = function(Capex, $event) {
    $event.currentTarget.style.display = "none";

var  boolCapexValue='false'
  if($scope.CapexPhaseColl1.length>1){
    for(var r =0;r<=$scope.CapexPhaseColl1.length;r++){
      if(Capex.CapexPhaseTotal==parseFloat($scope.CapexPhaseColl1[i].CapexPhaseTotal)){
          boolCapexValue='True';
      }
      ...
      $scope.CapexPhaseTotal2=($scope.CapexPhaseTotal2)-parseFloat(Capex.CapexPhaseTotal)/10000000;
        ...
        var t = $scope.CapexPhaseColl1.indexOf(Capex);
        $scope.CapexPhaseColl1.splice(t, 1);

        if ($scope.CapexPhaseColl1.length > 0 && $scope.CapexPhaseColl1.length <= 1) {
            $scope.CapexPhaseColl1[$scope.CapexPhaseColl1.length - 1].HideAddButton = false;
            $scope.CapexPhaseColl1[$scope.CapexPhaseColl1.length - 1].HideRemoveButton = true;
            $scope.showSubmit = true;
        } else {
            $scope.CapexPhaseColl1[$scope.CapexPhaseColl1.length - 1].HideAddButton = false;
            $scope.CapexPhaseColl1[$scope.CapexPhaseColl1.length - 1].HideRemoveButton = false;
        }
...
var currentRoWIndex = $scope.CapexPhaseColl1.length
    ...
    if ($scope.CapexPhaseColl1.length == 0) {

        setTimeout(function() {
            for (var i = 0; i < 1; i++) {
                var auditItemCopy = angular.copy(newAuditItem1);
                setAuditItem(auditItemCopy);
                currentRoWIndex = $scope.CapexPhaseColl1.length;
                //auditItemCopy.ObservationFor="Unscheduled";//1 line added by ketaki
                $scope.CapexPhaseColl1.push(auditItemCopy);
                $scope.CapexPhaseColl1[i].SrNo = currentRoWIndex + i + 1;
                $scope.CapexPhaseColl1[i].HideAddButton = false;
                // if (i > 3) {
                //     $scope.CapexPhaseColl1[i].HideAddButton = false;
                // }
                $scope.$apply();
    ...
    if ($scope.CapexPhaseColl1.length > 0) {
            for (var index = 0; index < $scope.CapexPhaseColl1.length; index++) {
                $scope.CapexPhaseColl1[index].SrNo = index + 1;
                $scope.CapexPhaseColl1[index].HideAddButton = true;
                if ($scope.isFormReadonly == true) {
                    $scope.CapexPhaseColl1[index].HideRemoveButton = true;
                } else {
                    $scope.CapexPhaseColl1[index].HideRemoveButton = false;
                }
                $scope.CapexPhaseColl1[index].CFSAAuditIDId = $scope.CapexPhaseColl1[index].ID;
                $scope.CapexPhaseColl1[index].ContractorSupervisorColl = $scope.contractorAgencyColl;
           
                if (index == $scope.CapexPhaseColl1.length - 1 && $scope.isFormReadonly == false) {
                    $scope.CapexPhaseColl1[index].HideAddButton = false;
                    if ($scope.CapexPhaseColl1.length == 1) {
                        $scope.CapexPhaseColl1[index].HideRemoveButton = true;
                    } else {
                        $scope.CapexPhaseColl1[index].HideRemoveButton = false;
        ...
        $scope.addAuditItem1 = function(auditObj) {
            if (auditObj != null && auditObj != undefined) {
                auditObj.HideAddButton = true;
            }
            var auditItemCopy1 = angular.copy(newAuditItem1);
            setAuditItem1(auditItemCopy1);
          ///  auditItemCopy.SrNo = auditObj.SrNo + 1;
            auditItemCopy1.HideAddButton = false;
            auditItemCopy1.HideRemoveButton = false;
            $scope.CapexPhaseColl1.push(auditItemCopy1);
            console.log($scope.CapexPhaseColl1);
