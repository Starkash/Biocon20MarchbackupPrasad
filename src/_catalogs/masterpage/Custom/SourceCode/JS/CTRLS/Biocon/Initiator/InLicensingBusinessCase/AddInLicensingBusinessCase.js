appOperations.controller("AddInLicensingBusinessCaseCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $routeParams, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    $scope.CurrDate = new Date();
    $scope.disabledFlag=false;
    // $scope.disabledMarketFlag=true;
    ...
    // $scope.ddlStrategyOL ="Inlicensing";
    ...
    //var strStrategyUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('StrategyMaster')/items?$select=*&$filter=Title eq 'Inlicensing'&$top=5000&$orderby=ID";
    var strSubStrategyUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('SubStrategyMaster')/items?$select=*,Strategy/Title,Strategy/Id&$expand=Strategy&$filter=Strategy/Title eq 'Inlicensing'&$orderby=ID";
   ...
   // change for  Email//var strOLBUCaseUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('InLicensingBusinessCase')/items?$select=*,Title,ID&$top=1&$orderby=Id desc";
    var strOLBUCaseUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('InLicensingBusinessCase')/items?&$select=Id,BusinessCaseDescription,Title,Modified,LapVersion,Counter,Editor/Id,Editor/Title,BusinessCaseName,InitiationDate,CaseStatus,CaseStage/Id,CaseStage/Title,Strategy/Id,Strategy/Title,SubStrategy/Id,SubStrategy/Title,ProductCategory/Id,ProductCategory/Title,ProductName/Id,ProductName/Title,DosageForm/Id,DosageForm/Title,Initiators/Id,Initiators/Title,Reviewers/Id,Reviewers/Title,Validators/Id,Validators/Title&$expand=Strategy,Editor,CaseStage,SubStrategy,ProductCategory,ProductName,DosageForm,Initiators,Reviewers,Validators&$top=5000&$orderby=Id desc";
   ...
   // var strBusinessCaseOLDocumentLinkUrlWorkFlow = _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getbytitle('InLicensingBusinessCaseDocuments')/items?$select=Id,VersionDescription,Title,Modified,CaseStage/Id,CaseStage/Title,Editor/Id,Editor/Title,Editor/EMail,LapVersion,Created,Author/Id,Author/Title,Author/EMail,CaseVersion,VersionType,InLicensingBusinessCaseId,InLicensingBusinessCase/Title,File/Name,File/ServerRelativeUrl&$expand=File,Author,CaseStage,Editor,InLicensingBusinessCase&$orderby=ID desc&$top=1000";
   var strBusinessCaseOLDocumentLinkUrlWorkFlow = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('InLicensingBusinessCaseDocuments')/items?$select=*&$top=1000&$orderby=ID"; // cascading
...
//
   ...
   $scope.DuplicateCheck= function chkDuplicates(arr,justCheck){
    ...
    $scope.GetProductcode();
        ...
        //Insert Data into Parent List (InLicensingBusinessCase)
        var strOLBusinessCaseURL = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('InLicensingBusinessCase')/items";
    ...
    $scope.LaunchSkuDetails = function () {
        ...
        for (var z = 0; z < $scope.FinalCountryLaunchColl.length; z++) {
            ...
            insertCountryBatch.push({
                reqUrl: _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('InLicensingLaunchDetails')/items",
                action: "ADD",
                async: false,
                data: {
                    __metadata: {
                    ...
                    InLicensingBusinessCaseId: $scope.OLBusinessCaseIntID
                }
            });
        }
    ...
    $scope.SkuDetails = function (AddedLaunchids) {
        ...
        for (var z = 0; z < $scope.FinalSKUColl.length; z++) {
            ...
            insertSKUDetailsBatch.push({
                reqUrl: _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('InLicensingSKUDetails')/items",
                action: "ADD",
                async: false,
                data: {
                    __metadata: {
                    ...
                    InLicensingLaunchDetailId: getLaunchID[0].LunchID,
                    InLicensingBusinessCaseId: $scope.OLBusinessCaseIntID
                }
            });
        }
        Logics.postBatchedData(_spPageContextInfo.webAbsoluteUrl, insertSKUDetailsBatch).then(function (insertedLaunchData) {
            ...
            if (index == $scope.FinalSKUColl.length) {
                ...
                //     + "<p>&nbsp;</p>"
                //     + "<p>Click "
                //     + "<a href='" + _spPageContextInfo.webAbsoluteUrl + "/SitePages/BusinessCase.aspx#!/ViewInLicensing/" + $scope.OLBUCaseColl[0].Id + "/" + $scope.BusinessCaseOLDocumentLinkUrlWorkFlowColl[0].DocID + "' target='_New'>here</a>"
