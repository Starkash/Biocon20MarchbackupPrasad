appOperations.controller("ChangeStageMultipleTenderCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
 ...
 $scope.MultipleBidDocLinkUrlWorkFlowColl= $scope.MultipleBidDocLinkUrlWorkFlowColl1;
        ...
        if($scope.MultipleBidColl.length>0){
            if ($scope.MultipleBidColl[0].CaseStatus == 'Data To Be Rework' && $scope.MultipleBidColl[0].CaseStage.Title == 'Under Validation') {
                var array = [];
            ...
            Logics.addData(strCopyBusinessCaseDocumentUrl, null).then(function (response) {
                ...
                Logics.getData(strBusinessCaseDocumentVersionUrl).then(function (docResponse) {
                    ...
                    if ($scope.filteredStageColl[0].Title == "Business Case Published") {
                        ...
                        var strInlicensingLaPUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('MultipleBidBusinessCase')/items(" + $scope.IntiateID + ")";
