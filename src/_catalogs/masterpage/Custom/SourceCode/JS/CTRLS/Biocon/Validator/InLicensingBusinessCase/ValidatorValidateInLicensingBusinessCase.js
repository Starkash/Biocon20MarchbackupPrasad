...
appOperations.controller("ValidatorValidateInLicensingBusinessCaseCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {

    if (Logics.getSharedData() == undefined) {
        $location.path('/ValidatorILDash');
    } else {
   ...
   var strInLicensingSKUDetailsUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('InLicensingSKUDetails')/items?&$select=Id,Title,Modified,Quantity,Pack,SkuUnit/Id,SkuUnit/Title,PackingType/Id,PackingType/Title,InLicensingLaunchDetail/Id,InLicensingLaunchDetail/Title,InLicensingBusinessCase/Id,InLicensingBusinessCase/Title&$expand=InLicensingBusinessCase,PackingType,SkuUnit,InLicensingLaunchDetail&$filter=InLicensingBusinessCase/ID eq '" + $scope.IntiateID + "'&$top=5000&$orderby=ID asc";
    ...
    var urlColl = [strInLicensingBusinessCaseUrl, strInLicensingLaunchDetailsUrl, strInLicensingSKUDetailsUrl, strBusinessCaseILDocumentLinkUrl, strInLicensingCommentsWorkflowHistoryUrl,strBusinessCaseILDocumentLinkUrlWorkFlow,strInLicensingCommentsWorkflowHistoryUrl2,strBusinessCaseILSupportingDocumentURL,strPackingMasterUrl,strCurrentStatusMasterUrl,strCurrencyMasterUrl,strProductMasterUrl];
...
$scope.CurrLogUserId=_spPageContextInfo.userDisplayName;
        ...
        $scope.InLicensingCommentsWorkflowHistoryColl = batchedData[4].d.results;
        $scope.BusinessCaseILDocumentLinkUrlWorkFlowColl =batchedData[5].d.results;
        $scope.InLicensingCommentsWorkflowHistoryColl2 =batchedData[6].d.results;
        $scope.InLicensingSupportingDocColl =batchedData[7].d.results;
          ...
          if($scope.InLicensingCommentsWorkflowHistoryColl.length>0)
            ...
            if ($scope.BusinessCaseILDocumentLinkColl.length > 0) {
                for (var s = 0; s < $scope.BusinessCaseILDocumentLinkColl.length; s++) {
                    ...
                    $scope.BusinessCaseILDocumentLinkColl[s].ServRel = location.origin + $scope.BusinessCaseILDocumentLinkColl[s].File.ServerRelativeUrl;
    ...
    //     var strNewBusinessCaseDocumentRelativeUrl = _spPageContextInfo.siteServerRelativeUrl + "/InLicensingBusinessCaseDocuments/" + $scope.Title +"-V" +(Math.floor($scope.BusinessCaseILDocumentLinkColl[0].CaseVersion) + 0.1).toFixed(1).toString() + ".xlsx";
    //     var strCopyBusinessCaseDocumentUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/getfilebyserverrelativeurl('" + $scope.BusinessCaseILDocumentLinkColl[0].File.ServerRelativeUrl + "')/copyto(strnewurl='" + strNewBusinessCaseDocumentRelativeUrl + "',boverwrite=true)";
    //     Logics.addData(strCopyBusinessCaseDocumentUrl, null).then(function (response) {
    //         console.log(response);    
    //         var strBusinessCaseDocumentVersionUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getbytitle('InLicensingBusinessCaseDocuments')/items?$select=Id,Title,CaseVersion,InLicensingBusinessCaseId,InLicensingBusinessCase/Title&$expand=InLicensingBusinessCase&$filter=InLicensingBusinessCaseId eq " + $scope.IntiateID + "&$orderby=Id desc&$top=1";
    ...
    //                     type: "SP.Data.InLicensingBusinessCaseDocumentsItem"
    //                 },
    //                 CaseVersion: (Math.floor($scope.BusinessCaseILDocumentLinkColl[0].CaseVersion) +0.1).toFixed(1)
    //             };
    ...
    $scope.onCreateVersion = function () {
      ...
      //  $scope.VersionDesc= 'Case Stage'+$scope.InLicensingBusinessCaseColl[0].CaseStage.Title+'; Comment'+$scope.CommentsWorkflowHistory+',Version Type'+"Minor"+',Member type'+ $scope.MemberType  ;        
        $scope.VersionDesc = 'Case Stage-' + $scope.InLicensingBusinessCaseColl[0].CaseStage.Title + ';'+'Comment-' + $scope.CommentsWorkflowHistory + ';'+'Version Type-' + "Minor" + ','+'Member type-' + $scope.MemberType;
        ...
        Logics.updateData(updateBusinessCaseRequestUrl, BcChangeStage).then(function (Changeresponse1) {
           ...
           // var strNewBusinessCaseDocumentRelativeUrl = _spPageContextInfo.siteServerRelativeUrl + "/InLicensingBusinessCaseDocuments/" + $scope.Title + "-V" + (parseFloat($scope.BusinessCaseILDocumentLinkColl[0].CaseVersion) + 0.1).toFixed(1).toString() + ".xlsx";
            ...
            Logics.addData(strCopyBusinessCaseDocumentUrl, null).then(function (response) {
                ...
                var strBusinessCaseDocumentVersionUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getbytitle('InLicensingBusinessCaseDocuments')/items?$select=Id,Title,VersionType,CaseVersion,InLicensingBusinessCaseId,InLicensingBusinessCase/Title&$expand=InLicensingBusinessCase&$filter=InLicensingBusinessCaseId eq " + $scope.IntiateID + "&$orderby=Id desc&$top=1";
                ...
                Logics.getData(strBusinessCaseDocumentVersionUrl).then(function (docResponse) {
                    ...
                    var BcVersion = {
                        ...
                        VersionType: "Minor",
                        VersionDescription: $scope.VersionDesc  //= 'Case Stage'+$scope.InLicensingBusinessCaseColl[0].CaseStage.Title+'; Comment'+$scope.CommentsWorkflowHistory+',Version Type'+"Minor"+',Member type'+ $scope.MemberType  ;        
    ...
    $scope.onValidatorOnHold = function () {
        ...
        var addCommentBusinessCaseRequest = {
           ...
           InLicensingBusinessCaseDocId: $scope.BusinessCaseILDocumentLinkColl[0].ID,
            ...
            CaseStage:$scope.InLicensingBusinessCaseColl[0].CaseStage.Title,
           MemberType: $scope.MemberType,
          VersionNo: (parseFloat($scope.BusinessCaseILDocumentLinkColl[0].CaseVersion)).toFixed(1),
