...
appOperations.controller("InitiatorILVsIHDashCtrl", function ($scope, $http, $filter, $q, $window, $timeout, $route, $routeParams, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    var strBusinessUSAndaLicencingUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('InLicensingVsInHouseBusinessCase')/items?$select=Id,AuthorId,Author/Title,Author/EMail,Delete,BusinessCaseDescription,Title,Initiators/Id,LapVersion,Initiators/Title,Counter,VersionNo,Modified,BusinessCaseName,InitiationDate,DosageForm/Id,DosageForm/Title,Strategy/Id,Strategy/Title,SubStrategy/Id,SubStrategy/Title,ProductNameId,ProductName/Title,CaseStage/Id,CaseStage/Title,CaseStatus&$expand=Strategy,Initiators,CaseStage,SubStrategy,DosageForm,ProductName,Author&$filter=Counter ne '0'&$top=5000&$orderby=ID desc"
    var strStrategyUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('StrategyMaster')/items?$select=*&$top=5000&$orderby=ID ";
    var strSubStrategyUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('SubStrategyMaster')/items?$select=*,Strategy/Title,Strategy/Id&$expand=Strategy&$top=5000&$orderby=ID ";
    var strInLicensingLaunchDetailsUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('InLicensingVsInHouseLaunchDetails')/items?$select=*,AuthorId,Author/Title,Author/EMail,PartnerId,Partner/Title,SubMarketId,SubMarket/Title,MarketId,Market/Title,SubMarketId,SubMarket/Title,CountryId,Country/Title,InLicensingVsInHouseBusinessCase/Title,InLicensingVsInHouseBusinessCaseId&$expand=InLicensingVsInHouseBusinessCase,Partner,SubMarket,Market,SubMarket,Country,Author&$top=5000&$orderby=ID asc";
    $scope.test=[];
        ...
        for (var i = 0; i < $scope.USAndaBusinessLicencingColl.length; i++) {
            ...
            for (var j = 0; j < $scope.USAndaBusinessLicencingColl[i].Initiators.results.length; j++) {
              ...
              if (_spPageContextInfo.webAbsoluteUrl == 'https://biocon.sharepoint.com/sites/SPCustomApplDev/BusinessCaseAutomation') {
                ...
                if (_spPageContextInfo.webAbsoluteUrl == 'https://biocon.sharepoint.com/sites/SPCustomAppl/BL/BusinessCaseAutomation') {
                    $scope.USAndaBusinessLicencingColl[i].DRCPowerBiLink = "https://app.powerbi.com/groups/9235f61b-adea-4690-aa06-b1297c156777/reports/1e4f6486-7189-4313-8bb2-9c3c6359b1e0/ReportSection4d40f6b94376d7660ac7?experience=power-bi&filter=BI_Input_Page%2FBusinessCaseId%20eq '" + $scope.USAndaBusinessLicencingColl[i].ID + "'";
...
//
        ...
        $scope.USAndaBusinessLicencingColl2=$scope.USAndaBusinessLicencingColl ;

        $scope.InLicensingLaunchDetailColl3=$scope.NewA1newfilteredarrayColl ;
        $scope.USAndaBusinessLicencingColl3=$scope.USAndaBusinessLicencingColl ;
        $scope.USAndaBusinessLicencingColl4=$scope.USAndaBusinessLicencingColl ;
        $scope.USAndaBusinessLicencingColl5=$scope.USAndaBusinessLicencingColl ;

         $scope.USAndaBusinessLicencingColl1 = $scope.USAndaBusinessLicencingColl;
            ...
            if (sbmarketname != null && sbmarketname != undefined && partnerSelec ==undefined) {
             ...
             if (pname != null && pname != undefined && Submarket==undefined) {
                 ...
                 for (var c = 0; c < $scope.DupInLicensingLaunchDetailColl.length; c++) {
                     ...
                     var filteredData = $filter('filter')($scope.USAndaBusinessLicencingColl2, function (item) {
         ...
         $scope.Submarketfilter = function (sbmarketname,partnerSelec) {
            ...
            if (sbmarketname != null && sbmarketname != undefined && partnerSelec !=undefined) {
                ...
                for (var c = 0; c < $scope.DupInLicensingLaunchDetailColl.length; c++) {
                    ...
                    var filteredData = $filter('filter')($scope.USAndaBusinessLicencingColl3, function (item) {
                        return item.ID == $scope.DupInLicensingLaunchDetailColl[c].InLicensingVsInHouseBusinessCaseId;
                    });
                    if (filteredData.length > 0) {
                        var col = {};
                ...
                $scope.USAndaBusinessLicencingColl = unique;
                $scope.test = unique//$scope.USAndaBusinessLicencingColl1;
            ...
            //
                ...
                $scope.USAndaBusinessLicencingColl1= $scope.USAndaBusinessLicencingColl;
                $scope.test = $scope.USAndaBusinessLicencingColl1;
             //   location.reload();
            }
