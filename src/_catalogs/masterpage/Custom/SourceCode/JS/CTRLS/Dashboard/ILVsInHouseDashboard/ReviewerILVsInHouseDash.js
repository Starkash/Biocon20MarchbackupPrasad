appOperations.controller("ReviewerILVsIHDashCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    localStorage.clear(); // New Change VD

    var strBusinessOutLicencingUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('InLicensingVsInHouseBusinessCase')/items?$select=Id,BusinessCaseDescription,VersionNo,LapVersion,Modified,Counter,Title,BusinessCaseName,InitiationDate,Initiators/Title,Initiators/Id,Reviewers/Id,Reviewers/Title,DosageForm/Id,DosageForm/Title,Strategy/Id,Strategy/Title,SubStrategy/Id,SubStrategy/Title,ProductName/Id,ProductName/Title,CaseStage/Id,CaseStage/Title,CaseStatus&$expand=Strategy,CaseStage,SubStrategy,DosageForm,ProductName,Initiators,Reviewers&$filter=Counter ne '0'&$top=5000&$orderby=ID desc"
    ...
    var strInLicensingLaunchDetailsUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('InLicensingVsInHouseLaunchDetails')/items?$select=*,PartnerId,Partner/Title,SubMarketId,SubMarket/Title,MarketId,Market/Title,SubMarketId,SubMarket/Title,CountryId,Country/Title,InLicensingVsInHouseBusinessCase/Title,InLicensingVsInHouseBusinessCaseId&$expand=InLicensingVsInHouseBusinessCase,Partner,SubMarket,Market,SubMarket,Country&$top=5000&$orderby=ID asc";
    $scope.test=[];
             ...
             if (OLFinalArray.length > 0) {
                        ...
                        for (var q = 0; q < OLFinalArray.length; q++) {
                            ...
                            $scope.A1newfilteredarrayColl = $filter('filter')($scope.InLicensingLaunchDetailColl, function (item) {
                                return (item.InLicensingVsInHouseBusinessCaseId == OLFinalArray[q].ID);
                            });
...
//         }
    ...
    $scope.ReviewForm = function (request) {
        ...
        if (request.Strategy.Title == 'In licensing IH Vs IL') {
            $location.path('/ReviewerReviewILVsIH');
        }
