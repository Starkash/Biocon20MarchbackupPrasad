...
appOperations.controller("InitiatorOLDashCtrl", function ($scope, $http, $filter, $q, $window, $timeout, $route, $routeParams, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    var strMultipleTenderBidUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('OutLicensingBusinessCase')/items?$select=Id,Delete,BusinessCaseDescription,LapVersion,Title,Initiators/Id,Initiators/Title,VersionNo,Modified,Counter,BusinessCaseName,InitiationDate,DosageForm/Id,DosageForm/Title,Strategy/Id,Strategy/Title,SubStrategy/Id,SubStrategy/Title,ProductNameId,ProductName/Title,CaseStage/Id,CaseStage/Title,CaseStatus&$expand=Strategy,Initiators,CaseStage,SubStrategy,DosageForm,ProductName&$filter=Counter ne '0'&$top=5000&$orderby=ID desc"
                ...
                for (var j = 0; j < $scope.MultipleTenderBidColl[i].Initiators.results.length; j++) {
                    ...
                    if (userExist == true) {
                        ...
                        if (_spPageContextInfo.webAbsoluteUrl == 'https://biocon.sharepoint.com/sites/SPCustomApplUAT/BusinessCaseAutomation') {
                          //  $scope.MultipleTenderBidColl[i].V2VPowerBiLink = "https://app.powerbi.com/groups/02afbc0f-af46-45ca-99cb-35370af19987/reports/8964d82f-f633-42aa-9ebf-8939905f13c7/ReportSection?experience=power-bi&filter=OutLicensingBusinessCase/Id eq " + $scope.MultipleTenderBidColl[i].ID + "";
                            $scope.MultipleTenderBidColl[i].V2VPowerBiLink="https://app.powerbi.com/groups/02afbc0f-af46-45ca-99cb-35370af19987/reports/d47160e0-13c8-4662-892f-2621c9c8009b/ReportSection?experience=power-bi&filter=OutLicensingBusinessCase/Id eq "+$scope.MultipleTenderBidColl[i].ID+"";
                    
    }

                        if (_spPageContextInfo.webAbsoluteUrl == 'https://biocon.sharepoint.com/sites/SPCustomAppl/BL/BusinessCaseAutomation') {
                          //  $scope.MultipleTenderBidColl[i].V2VPowerBiLink = "https://app.powerbi.com/groups/da2b48bc-8887-4ad2-a4cb-3b7d689a4207/reports/c1af407c-0385-43de-9825-9e1d3fe0063c/ReportSection?experience=power-bi&filter=OutLicensingBusinessCase/Id eq " + $scope.MultipleTenderBidColl[i].ID + "";
                      
                            $scope.MultipleTenderBidColl[i].V2VPowerBiLink="https://app.powerbi.com/groups/da2b48bc-8887-4ad2-a4cb-3b7d689a4207/reports/88a29da1-7f76-4fb3-8e13-79500393dea4/ReportSection?experience=power-bi&filter=OutLicensingBusinessCase/Id eq "+$scope.MultipleTenderBidColl[i].ID+"";
        ...
        if (confirmation) {
            ...
            $scope.IntiateID = data.Id;
                ...
                $scope.isLoading=true;
                      ...
                      var strBusinessCaseOLSupportingDocumentURL = _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getbytitle('OutLicensingSupportingDocument')/items?$select=Id,Title,OutLicensingBusinessCaseId,OutLicensingBusinessCase/Title&$expand=OutLicensingBusinessCase&$filter=OutLicensingBusinessCaseId eq '" + $scope.IntiateID + "'&$top=5000&$orderby=ID asc";
                        ...
                        if ($scope.BusinessCaseOLSupportingDocumentColl1.length > 0) {
                            for (var d = 0; d< $scope.BusinessCaseOLSupportingDocumentColl1.length; d++) {
                                        ...
                                        var reqUrl= _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/OutLicensingSupportingDocument/items(" + $scope.BusinessCaseOLSupportingDocumentColl1[d].Id + ")"
                                   
                    

                                        var  data= {
                                            '__metadata': { 'type': 'SP.Data.OutLicensingSupportingDocumentItem' },
