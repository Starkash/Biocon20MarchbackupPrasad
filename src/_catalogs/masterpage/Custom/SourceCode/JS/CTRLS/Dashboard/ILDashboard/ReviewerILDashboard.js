appOperations.controller("ReviewerILDashCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    localStorage.clear(); // New Change VD

    var strBusinessInLicencingUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('InLicensingBusinessCase')/items?$select=Id,BusinessCaseDescription,VersionNo,LapVersion,Counter,Modified,Title,BusinessCaseName,InitiationDate,Reviewers/Id,Reviewers/Title,DosageForm/Id,DosageForm/Title,Strategy/Id,Strategy/Title,SubStrategy/Id,SubStrategy/Title,ProductNameId,ProductName/Title,CaseStage/Id,CaseStage/Title,CaseStatus&$expand=Strategy,CaseStage,SubStrategy,DosageForm,ProductName,Reviewers&$filter=Counter ne '0'&$top=5000&$orderby=ID desc"
...
$scope.ifallblank=function(){
    ...
    if(($scope.ddlStatus=="" || $scope.ddlStatus=='Under Internal Review' ||$scope.ddlStatus==undefined) && ($scope.ddlPartner==undefined || $scope.ddlPartner=="" ) &&
...
//                  $scope.InBusinessLicencingLicencingColl= $scope.InBusinessLicencingLicencingColl1
...
//         }
        ...
        // $scope.BlankInBusinessLicencingLicencingColl = $filter('filter')($scope.InBusinessLicencingLicencingColl, function (responseLine) {
