...
// if all blank then 
       ...
       // 8318831586 -- rajman 
        ...
        $scope.Partnerfilter = function (pname,Submarket) {
            if(OLFinalArray.length>0){
                for(var w=0;w<OLFinalArray.length;w++){
                $scope.OutLicensingLaunchDetailColl2 = $filter('filter')($scope.OutLicensingLaunchDetailColl2, function (item) {
                    return item.OutLicensingBusinessCaseId == OLFinalArray[w].Id;
                });
        ...
        $scope.Submarketfilter = function (sbmarketname,partnerSelec) {
            ...
            if(OLFinalArray.length>0){
                for(var w=0;w<OLFinalArray.length;w++){
                $scope.OutLicensingLaunchDetailColl3 = $filter('filter')($scope.OutLicensingLaunchDetailColl3, function (item) {
                    return item.OutLicensingBusinessCaseId == OLFinalArray[w].Id;
                });
            }
