appOperations.controller("InitiatorUSMOWANDADashCtrl", function ($scope, $http, $filter, $q, $window, $timeout, $route, $routeParams, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    ...
    var strInLicensingLaunchDetailsUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('AndaLicensingLaunchDetails')/items?$select=*,PartnerId,Partner/Title,SubMarketId,SubMarket/Title,MarketId,Market/Title,SubMarketId,SubMarket/Title,CountryId,Country/Title,AndaBusinessCase/Title,AndaBusinessCaseId&$expand=AndaBusinessCase,Partner,SubMarket,Market,SubMarket,Country&$top=5000&$orderby=ID ";
    $scope.test=[];
        ...
        if ($scope.AndaBusinessLicencingColl.length > 0) {
            ...
            for (var j = 0; j < $scope.AndaBusinessLicencingColl[i].Initiators.results.length; j++) {
                ...
                if (userExist == true) 
                {
                    ...
                    if (_spPageContextInfo.webAbsoluteUrl == 'https://biocon.sharepoint.com/sites/SPCustomApplUAT/BusinessCaseAutomation') {
                        $scope.AndaBusinessLicencingColl[i].DRCPowerBiLink = "https://app.powerbi.com/groups/d71e961b-fc31-482b-8e38-8d2e6587e058/reports/4f0461db-789e-46aa-9b36-41bb8412d234/ReportSection?experience=power-bi&filter=BI_Input_Page%2FBusinessCaseId eq '" + $scope.AndaBusinessLicencingColl[i].ID + "'";
                    }
    
                    if (_spPageContextInfo.webAbsoluteUrl == 'https://biocon.sharepoint.com/sites/SPCustomAppl/BL/BusinessCaseAutomation') {
                        $scope.AndaBusinessLicencingColl[i].DRCPowerBiLink = "https://app.powerbi.com/groups/da2b48bc-8887-4ad2-a4cb-3b7d689a4207/reports/557453ae-b1b3-42fe-9490-8fce8f72a45f/ReportSection?experience=power-bi&filter=OutLicensingBusinessCase/Id eq " + $scope.AndaBusinessLicencingColl[i].ID + "";
