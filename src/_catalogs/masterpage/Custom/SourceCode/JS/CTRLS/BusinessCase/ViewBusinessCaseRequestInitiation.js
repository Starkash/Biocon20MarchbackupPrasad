...
appOperations.controller("ViewBusinessCaseInitiationCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    ...
    var strProductNameUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('ProductMaster')/items?$select=ProductCategoryName/Title,ProductCategoryName/Id&$expand=ProductCategoryName&$top=5000&$orderby=ID desc";  
    ...
    Logics.getBatchedData(_spPageContextInfo.webAbsoluteUrl, urlColl).then(function (batchedData) {
        ...
        if ($scope.BcRquestColl.length>0) {
            ...
            if($scope.businessCaseDocumentColl.length>0){
                ...
                $scope.MajorV= Math.floor($scope.BusinessCaseDocumentLinkColl[0].CaseVersion);
                $scope.MinorV= parseFloat($scope.BusinessCaseDocumentLinkColl[0].CaseVersion).toFixed(1);
            ...
            if($scope.BusinessCaseDocumentLinkColl!=null && $scope.BusinessCaseDocumentLinkColl!='' && $scope.BusinessCaseDocumentLinkColl!=undefined)
            {
            ...
            // if($scope.BcRquestColl[0].CFTuser.results!=null && $scope.BcRquestColl[0].CFTuser.results!='' && $scope.BcRquestColl[0].CFTuser.results!=undefined)
            // {
            ...
            if($scope.BcRquestColl[0].Dosage!=null && $scope.BcRquestColl[0].Dosage!='' && $scope.BcRquestColl[0].Dosage!=undefined){
