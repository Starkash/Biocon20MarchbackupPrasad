...
appOperations.controller("ValidatorCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
        var strRollURL = _spPageContextInfo.webAbsoluteUrl + "/_api/lists/getbytitle('RoleMaster')/items?$select=*,Member/Title,Member/Name,Member/EMail&$expand=Member&$filter=Member/EMail eq '" + _spPageContextInfo.userEmail + "'&$orderby=ID desc&$top=5000";
            ...
            if (Logics.getSharedData() == undefined) {
                $location.path("/ValidatorDashboard");
            } {
            ...
            Logics.getBatchedData(_spPageContextInfo.webAbsoluteUrl, urlColl).then(function (batchedData) {
               ...
               $.ajax({
                ...
                if ($scope.BcRquestColl.length > 0) {
                    ...
                    Businesscaseid= $scope.BcRquestColl[0].BusinessCaseID;
    ...
    $scope.onValidatorSubmit = function () {
        ...
        if ($scope.ValidatorComments == '' || $scope.ValidatorComments == undefined) {
            alert("Please Enter Validator Comments")
            return false;
        }
        $scope.BusinessCaseStages="Validation Completed";

        BcValidator={
            __metadata: {
                type: "SP.Data.BusinessCaseRequestsListItem"
            },
            ValidatorComments:$scope.ValidatorComments,
            ValidatedDate:new Date(),
            BusinessCaseStages:$scope.BusinessCaseStages   

        }

        BcValidatorUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('BusinessCaseRequests')/items(" + $scope.IntiateID + ")",

        Logics.updateData(BcValidatorUrl, BcValidator).then(function (response) {
