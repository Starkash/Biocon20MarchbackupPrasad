...
appOperations.controller("ELTReviewerILDashboardCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    ...
    var strBusinessInLicencingUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('InLicensingBusinessCase')/items?$select=Id,BusinessCaseDescription,VersionNo,LapVersion,Modified,Title,BusinessCaseName,InitiationDate,Counter,Initiators/Title,Initiators/Id,Reviewers/Id,Reviewers/Title,Validators/Id,Validators/Title,DosageForm/Id,DosageForm/Title,Strategy/Id,Strategy/Title,SubStrategy/Id,SubStrategy/Title,ProductNameId,ProductName/Title,CaseStage/Id,CaseStage/Title,CaseStatus&$expand=Strategy,CaseStage,SubStrategy,DosageForm,ProductName,Initiators,Reviewers,Validators&$filter=Counter ne '0'&$top=5000&$orderby=ID desc"
    var strStrategyUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('StrategyMaster')/items?$select=*&$top=5000&$orderby=ID ";
    var strSubStrategyUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('SubStrategyMaster')/items?$select=*&$top=5000&$orderby=ID ";
    var strOutLicensingLaunchDetailsUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('InLicensingLaunchDetails')/items?$select=*,PartnerId,Partner/Title,SubMarketId,SubMarket/Title,MarketId,Market/Title,SubMarketId,SubMarket/Title,CountryId,Country/Title,InLicensingBusinessCase/Title,InLicensingBusinessCaseId&$expand=InLicensingBusinessCase,Partner,SubMarket,Market,SubMarket,Country&$top=5000&$orderby=ID ";
    $scope.test=[];

    var urlColl = [strBusinessInLicencingUrl, strStrategyUrl, strSubStrategyUrl,strOutLicensingLaunchDetailsUrl];
    Logics.getBatchedData(_spPageContextInfo.webAbsoluteUrl, urlColl).then(function (batchedData) {
             ...
             if (pname != null && pname != undefined && Submarket==undefined) {
                 ...
                 for (var c = 0; c < $scope.DupOutLicensingLaunchDetailColl.length; c++) {
                     ...
                     var filteredData = $filter('filter')($scope.OutBusinessLicencingColl2, function (item) {
                         return item.ID == $scope.DupOutLicensingLaunchDetailColl[c].InLicensingBusinessCaseId;
                     });
        ...
        if ($scope.OutLicensingLaunchDetailColl.length > 0) {
             ...
             for (var i = 0; i < $scope.OutLicensingLaunchDetailColl.length; i++) {
                 ...
                 col.OutLicensingBusinessCaseId = $scope.OutLicensingLaunchDetailColl[i].InLicensingBusinessCase.Id;
 ...
 $scope.test = $scope.InBusinessLicencingLicencingColl1;
    ...
    $scope.ViewValidateForm = function (request,ELTReviewerAPIDash) {
        ...
        if (request.Strategy.Title == 'Inlicensing') {
            $location.path('/ReviewerInLicensingBusinessCaseView');          //ReviewerReviewOutLicensingBusinessCase
        }
    }
