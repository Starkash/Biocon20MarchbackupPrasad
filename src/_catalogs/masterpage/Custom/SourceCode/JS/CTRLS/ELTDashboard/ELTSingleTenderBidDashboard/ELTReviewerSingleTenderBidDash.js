...
//                         $scope.APIColl[i].V2VPowerBiLink = "https://app.powerbi.com/groups/7df8b98c-0bdb-41d0-9056-94f37dc4121c/reports/a1a76d85-41f0-4b06-8c03-2525accb4e0a/ReportSection?experience=power-bi&filter=OutLicensingBusinessCase/Id eq " + $scope.APIColl[i].ID + "";
//                     }
//                     if (_spPageContextInfo.webAbsoluteUrl == 'https://biocon.sharepoint.com/sites/SPCustomApplUAT/BusinessCaseAutomation') {
//                         $scope.APIColl[i].V2VPowerBiLink = "https://app.powerbi.com/groups/02afbc0f-af46-45ca-99cb-35370af19987/reports/8964d82f-f633-42aa-9ebf-8939905f13c7/ReportSection?experience=power-bi&filter=SingleBidBusinessCase/Id eq " + $scope.APIColl[i].ID + "";
//                     }

//                     if (_spPageContextInfo.webAbsoluteUrl == 'https://biocon.sharepoint.com/sites/SPCustomAppl/BL/BusinessCaseAutomation') {
                       
//                   //  $scope.APIColl[i].V2VPowerBiLink = "https://app.powerbi.com/groups/02afbc0f-af46-45ca-99cb-35370af19987/reports/2bf91bf3-79ad-4174-9f9b-7f3312c46ada/ReportSection?experience=power-bi&filter=OutLicensingBusinessCase/Id eq " + $scope.APIColl[i].ID + "";
//                     $scope.APIColl[i].V2VPowerBiLink = "https://app.powerbi.com/groups/da2b48bc-8887-4ad2-a4cb-3b7d689a4207/reports/c1af407c-0385-43de-9825-9e1d3fe0063c/ReportSection?experience=power-bi&filter=SingleBidBusinessCase/Id eq " + $scope.APIColl[i].ID + "";
...
//     //view
//     $scope.ViewReviewerForm = function (request,ReviewerAPIDash) {
//         Logics.setSharingDataTwoDash(request,ReviewerAPIDash);
...
appOperations.controller("ELTReviewerSingleTenderBidDashCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    ...
    var strStrategyUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('StrategyMaster')/items?$select=*&$top=5000&$orderby=ID ";
    var strSubStrategyUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('SubStrategyMaster')/items?$select=*&$top=5000&$orderby=ID ";
    var strOutLicensingLaunchDetailsUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('SingleBidLaunchDetails')/items?$select=*,PartnerId,Partner/Title,SubMarketId,SubMarket/Title,MarketId,Market/Title,SubMarketId,SubMarket/Title,CountryId,Country/Title,SingleBidBusinessCase/Title,SingleBidBusinessCaseId&$expand=SingleBidBusinessCase,Partner,SubMarket,Market,SubMarket,Country&$top=5000&$orderby=ID ";
    $scope.test=[];
    ...
    Logics.getBatchedData(_spPageContextInfo.webAbsoluteUrl, urlColl).then(function (batchedData) {
             ...
             if (pname != null && pname != undefined && Submarket==undefined) {
                 ...
                 for (var c = 0; c < $scope.DupOutLicensingLaunchDetailColl.length; c++) {
                     ...
                     var filteredData = $filter('filter')($scope.OutBusinessLicencingColl2, function (item) {
                         return item.ID == $scope.DupOutLicensingLaunchDetailColl[c].SingleBidBusinessCaseId;
                     });
        ...
        if ($scope.OutLicensingLaunchDetailColl.length > 0) {
             ...
             for (var i = 0; i < $scope.OutLicensingLaunchDetailColl.length; i++) {
                 ...
                 col.OutLicensingBusinessCaseId = $scope.OutLicensingLaunchDetailColl[i].SingleBidBusinessCase.Id;
    ...
    //     Logics.setSharingData(request);
    //     if (request.Strategy.Title == 'Tender Bid single Scenario') {
    //         $location.path('/ValidateSingleTenderView');
    //     }
    // }
    ...
    $scope.ValidateForm = function (request) {
        ...
        if (request.Strategy.Title == 'Tender Bid single Scenario') {
            $location.path('/ValidateSingleTender');
        }
