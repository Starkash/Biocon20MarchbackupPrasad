...
appOperations.controller("ReviewerCtrl", function ($scope, $http, $filter, $q, $timeout, $route, $interval, NgTableParams, Logics, Utilities, $uibModal, $uibModalStack, $rootScope, $location, $dialogs) {
    ...
    Logics.getBatchedData(_spPageContextInfo.webAbsoluteUrl, urlColl).then(function (batchedData) {
            ...
            if ($scope.BcRquestColl.length > 0) {
                ...
                // if ($scope.BcRquestColl[0].CFTuser.results.length > 0) {
    ...
    $scope.onReviewerSubmit = function () {
        ...
        if ($scope.ReviewerComments == '' || $scope.ReviewerComments == undefined) {
            alert("Please Enter Reviewer Comments")
            return false;
        }

        $scope.BusinessCaseStages = "Review Completed";

        BusinessCaseReview = {
            __metadata: {
                type: "SP.Data.BusinessCaseRequestsListItem"
            },
            ReviewerComments: $scope.ReviewerComments,
            ReviewedDate: new Date(),
            BusinessCaseStages: $scope.BusinessCaseStages
        }

        BcReviewerUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('BusinessCaseRequests')/items(" + $scope.IntiateID + ")",

            Logics.updateData(BcReviewerUrl, BusinessCaseReview).then(function (response) {
    ...
    $scope.onReviewerReject = function () {
        ...
        $scope.BusinessCaseStages = "Rejected";
        ...
        ReviewerRejUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('BusinessCaseRequests')/items(" + $scope.IntiateID + ")",

            Logics.updateData(ReviewerRejUrl, ReviewerReject).then(function (response) {
                console.log(response);
    ...
    $scope.onReviewerRework = function () {
        ...
        $scope.BusinessCaseStages = "SendBack/Rework";
        ...
        BcReviewerUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/Lists/getbytitle('BusinessCaseRequests')/items(" + $scope.IntiateID + ")",
            ...
            Logics.updateData(BcReviewerUrl, BusinessCaseReview).then(function (response) {
                ...
                alert("Form Send Back for Rework Successfully!!");
                $location.path("/ReviewerDashboard");
            });
    }
